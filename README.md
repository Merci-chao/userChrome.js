...
| `floatingBackdropBlurriness` | How blurry the background of items covering the Tabs Bar is when scrolling, available when `tabsUnderControlButtons` is `2` and both `floatingBackdropClip` & `nativeWindowStyle` are `false`. The minimum value is `0`. Not available on Firefox 115. |
