...
- This script is more like a comprehensive tab extension than a rough multi-row tweak. It consolidates thousands of lines of logic and styling into a single file, yet remains as manageable as a typical `userChrome.js` script. It is a hard-core implementation built with perfection and performance in mind. The number of lines is never a concern — just as [Tab Mix Plus](https://onemen.github.io/tabmixplus-docs) could never be achieved with only a few hundred lines. However, if you prefer something simple, fully understandable, and easy to control — providing a stronger sense of safety — this may not be your favorite choice.
...
| `previewPanelShifted` | Shift the preview panel when there are multiple rows, reducing the effect of the panel blocking items in the rows underneath. Affects tabs only when `previewPanelNoteEditable` is `true`.<ul><li>`0` - never</li><li>`1` - for groups</li><li>`2` - for tabs</li><li>`3` - for both</li></ul> |
...
| ------------- | ------------- |
| <span id="multiselectStacking">`browser.tabs.dragDrop.multiselectStacking`</span> | Enable tab stacking when dragging tabs. On Firefox 145 and below (including 115), create a new preference with this name to toggle. |
