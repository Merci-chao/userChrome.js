...
2. Install the script loader (*userChrome.js*). Skip to step 3 if you already have one in use. There are several script loaders available:
	- [*MrOtherGuy/fx-autoconfig*](https://github.com/MrOtherGuy/fx-autoconfig) <details><summary>Installation Steps</summary><p>📝 Note: These steps are provided as a simple reference. For complete and accurate instructions, please consult the official page.</p><ol><li><p>Get the source files needed for setup from the <a href="https://github.com/MrOtherGuy/fx-autoconfig/archive/refs/heads/master.zip">direct link</a>, or from the page above:</p><ul><li>Click the green <em>Code</em> button.</li><li>Select <em>Download ZIP</em>.</li></ul></li><li><p>Open <em><a href="https://support.mozilla.org/kb/use-troubleshooting-information-page-fix-firefox">about:support</a></em> in Firefox to identify both the program folder and the profile folder:</p><ul><li>Under <em>Application Binary</em>, find the path to <code>firefox.exe</code> (e.g., <code>C:\Program Files\Mozilla Firefox</code>).</li><li>Under <em>Profile Folder</em>, click <em>Open Folder</em> to access it.</li></ul></li><li><p>Extract `fx-autoconfig-master.zip` and place the required files into the correct folders:</p><ul><li>From <code>fx-autoconfig-master\program</code>, place <code>config.js</code> and <code>defaults</code> directly into the program folder (admin rights may be needed).</li><li>From <code>fx-autoconfig-master\profile</code>, place <code>chrome</code> directly into the profile folder.</li></ul></li><li><p>If you already have scripts in use:</p><ul><li>Option A: Move all <code>*.uc.js</code> files from the <code>chrome</code> folder into the <code>chrome\JS</code> sub-folder.</li><li>Option B: Edit <code>chrome\utils\chrome.manifest</code> and change <code>content userscripts <mark>../JS/</mark></code> to <code>content userscripts <mark>../</mark></code>. This will load scripts directly from the <code>chrome</code> folder.</li></ul></li><li><p>Click the <em>Clear startup cache…</em> button in the top right of <em>about:support</em> to restart Firefox.</p></li><li><p>Confirm that the setup is working correctly:</p><ul><li>Check that a new menu item <em>userScripts</em> appears under the <em>Tools</em> menu (<code>Alt</code>+<code>T</code>).</li></ul></li><li><p>Additionally, enable the automatic update checking feature:</p><ul><li>Open <a href="https://support.mozilla.org/kb/about-config-editor-firefox"><em>about:config</em></a>, paste <code>userChromeJS.updates.update-check.enabled</code> into the search box, and click the <em>Add</em> (+) button to create a boolean preference set to <code>true</code>.</li></ul></li></ol></details>
	- [*Firefox Scripts*](https://onemen.github.io/tabmixplus-docs/other/installation/#install-firefox-scripts) (forked from *xiaoxiaoflood/firefox-scripts*) <details><summary>Installation Steps</summary><p>📝 Note: These steps are provided as a simple reference. For complete and accurate instructions, please consult the official page.</p><ol><li><p>Get the source files needed for setup from the following direct links, or from the page above:</p><ul><li><a href="https://github.com/onemen/TabMixPlus/releases/download/dev-build/fx-folder.zip">Configuration files</a></li><li><a href="https://github.com/onemen/TabMixPlus/releases/download/dev-build/utils.zip">Utils</a></li></ul></li><li><p>Open <em><a href="https://support.mozilla.org/kb/use-troubleshooting-information-page-fix-firefox">about:support</a></em> in Firefox to identify both the program folder and the profile folder:</p><ul><li>Under <em>Application Binary</em>, find the path to <code>firefox.exe</code> (e.g., <code>C:\Program Files\Mozilla Firefox</code>).</li><li>Under <em>Profile Folder</em>, click <em>Open Folder</em> to access it.</li></ul></li><li><p>Extract the required files into the correct folders:</p><ul><li>Open <code>fx-folder.zip</code> and from <code>fx-folder</code>, extract <code>config.js</code> and <code>defaults</code> directly into the program folder (admin rights may be needed).</li><li>Create the <code>chrome</code> folder in the profile folder if not exist.</li><li>Create the <code>utils</code> folder in the <code>chrome</code> folder if not exist.</li><li>Open <code>utils.zip</code>, extract all files directly into the `utils` folder.</li></ul></li><li><p>Click the <em>Clear startup cache…</em> button in the top right of <em>about:support</em> to restart Firefox.</p></li><li><p>Confirm that the setup is working correctly:</p><ul><li><p>Open <a href="https://support.mozilla.org/kb/about-config-editor-firefox"><em>about:config</em></a> and search for <code>userChromeJS.enabled</code>. Verify that the preference exists without a delete button (🗑️) on the right. If the delete button is present, click it to delete the preference, then restart Firefox and recheck.</p></li></ul></li></ol><p>⛔ DO NOT install <em>Tab Mix Plus</em> as it will cause conflict.</p></details>
 	- [*alice0775/userChrome.js*](https://github.com/alice0775/userChrome.js) (Japanese) <details><summary>Installation Steps</summary><p>📝 Note: These steps are provided as a simple reference. For complete and accurate instructions, please consult the official page.</p><ol><li><p>Open <em><a href="https://support.mozilla.org/kb/use-troubleshooting-information-page-fix-firefox">about:support</a></em> in Firefox to identify both the program folder and the profile folder:</p><ul><li>Under <em>Application Binary</em>, find the path to <code>firefox.exe</code> (e.g., <code>C:\Program Files\Mozilla Firefox</code>).</li><li>Under <em>Profile Folder</em>, click <em>Open Folder</em> to access it.</li></ul></li><li><p>Go to <a href="https://github.com/alice0775/userChrome.js">the GitHub page</a>, find the *Go to file* search box on the left of the green *Code* button, or simply press the <code>T</code> key to focus it:</p><ul><li>Type <code>userChrome.js</code> and choose the latest version, typically called <em>xxx/userChrome.js</em>. Click the 📥 button to download the file.</li><li>Type <code>install_folder</code> and choose the latest version, typically called <em>xxx/install_folder</em>. Download both <code>config.js</code> and <code>defaults/pref/config-prefs.js</code>.</li></ul><p>⚠️ Creating the files by copy & paste or other methods may cause incorrect encoding.</p></li><li><p>Place the required files into the program folder:</p><ul><li>Place <code>config.js</code> into the program folder (admin rights may be needed).</li><li>Place <code>config-prefs.js</code> into the <code>defaults\pref</code> folder under the program folder.</li></ul></li><li><p>Place the required files into the profile folder:</p><ul><li>Create the <code>chrome</code> folder in the profile folder if not exist.</li><li>Place <code>userChrome.js</code> into the <code>chrome</code> folder.</li></ul></li><li><p>Click the <em>Clear startup cache…</em> button in the top right of <em>about:support</em> to restart Firefox.</p></li><li><p>Confirm that the setup is working correctly:</p><ul><li>Press <code>Ctrl</code>+<code>Shift</code>+<code>J</code> to open the <em>Browser Console</em>. Enable the <em>Logs</em> filter, enter <code>getScripts</code> in the <em>Filter Output</em>, and verify that the related logs appear.<details><summary>Screenshot</summary><img src="https://cdn.jsdelivr.net/gh/Merci-chao/userChrome.js@main/screenshots/alice-scriptloader-en.png"></details></li></ul></li></ol></details>
