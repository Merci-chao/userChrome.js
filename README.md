...
| `disableDragToPinOrUnpin` | Disable tab pinning/unpinning via drag-and-drop in the same window (not available on Firefox 115). This setting will be removed if an official one is introduced. |
...
**Version 3.5.2**
- Bug fix: dragged tabs could not be moved to the intended position if it is pressed against to the edge.
