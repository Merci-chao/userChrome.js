...
- Supports general *userChrome.js* script loaders, such as:
	- [*onemen/firefox-scripts*](https://github.com/onemen/firefox-scripts)
...
2. Install the script loader (*userChrome.js*). Skip to step 3 if you already have one in use. There are several script loaders available:
	- [*onemen/firefox-scripts*](https://github.com/onemen/firefox-scripts) (with installer and updater developed with vibe-coding) <p>🚧 [UNDER MAINTENANCE](https://github.com/onemen/firefox-scripts/issues/167) 🚧</p><details><summary><del>Installation Steps (Installer)</del></summary><ol><li><p>Download the installer for your OS from the [official page](https://github.com/onemen/firefox-scripts/releases) or from the direct links below:</p><ul><li>[`installer_win.exe`](https://github.com/onemen/firefox-scripts/releases/download/latest/installer_win.exe) — Windows</li><li>[`installer_linux`](https://github.com/onemen/firefox-scripts/releases/download/latest/installer_linux) — Linux (x86_64)</li><li>[`installer_linux_aarch64`](https://github.com/onemen/firefox-scripts/releases/download/latest/installer_linux_aarch64) — Linux (ARM64)</li></ul></li><li><p>Run the installer while Firefox is running.</p></li><li><p>In the installation page shown in Firefox, install the required components (admin rights may be needed):</p><img src="https://cdn.jsdelivr.net/gh/Merci-chao/userChrome.js@main/screenshots/firefox-scripts-installer.png"></li><li><p>Verify that the installation works correctly:</p><ul><li>Paste `chrome://firefox-scripts/content/ui/updater.html` into the address bar and press `Enter`.</li><li>Confirm that the page is displayed and all items are green in state.</li></ul></li><li><p>When an update is released, update the components in the updater page shown in Firefox:</p><img src="https://cdn.jsdelivr.net/gh/Merci-chao/userChrome.js@main/screenshots/firefox-scripts-updater.png"></li></ol></details><details><summary><del>Installation Steps (Manual)</del></summary><p>📝 Note: These steps are provided as a simple reference. For complete and accurate instructions, please consult the official page above.</p><ol><li><p>Get the source files needed for setup from the [official page](https://github.com/onemen/firefox-scripts/releases), or from the following direct links:</p><ul><li>[`fx-folder.zip`](https://github.com/onemen/firefox-scripts/releases/download/latest/fx-folder.zip)</li><li>[`utils.zip`](https://github.com/onemen/firefox-scripts/releases/download/latest/utils.zip)</li></ul></li><li><p>Open *[about:support](https://support.mozilla.org/kb/use-troubleshooting-information-page-fix-firefox)* in Firefox to identify both the program folder and the profile folder:</p><ul><li>Under *Application Binary*, find the path to `firefox.exe` (e.g., `C:\Program Files\Mozilla Firefox`).</li><li>Under *Profile Folder*, click *Open Folder* to access it.</li></ul></li><li><p>Extract the required files into the correct folders:</p><ul><li>Open `fx-folder.zip` and from `fx-folder`, extract `config.js` and `defaults` directly into the program folder (admin rights may be needed).</li><li>Create the `chrome` folder in the profile folder if not exist.</li><li>Create the `utils` folder in the `chrome` folder if not exist.</li><li>Open `utils.zip`, extract all files directly into the `utils` folder.</li></ul></li><li><p>Click the *Clear startup cache…* button in the top right of *about:support* to restart Firefox.</p></li><li><p>Confirm that the setup is working correctly:</p><ul><li>Paste `chrome://firefox-scripts/content/ui/updater.html` into the address bar and press `Enter`.</li><li>If it shows *File not found*, wait for a minute and try again.</li><li>Confirm that the page is displayed and all items are green in state.</li></ul></li><li><p>When an update is released, update the components in the updater page shown in Firefox:</p><img src="https://cdn.jsdelivr.net/gh/Merci-chao/userChrome.js@main/screenshots/firefox-scripts-updater.png"></li></ol></details>
	- [*onemen/firefox-scripts*](https://onemen.github.io/tabmixplus-docs/other/installation/#install-firefox-scripts) (traditional) <details><summary>Installation Steps</summary><p>📝 Note: These steps are provided as a simple reference. For complete and accurate instructions, please consult the official page above.</p><ol><li><p>Get the source files needed for setup from the [official page](https://onemen.github.io/tabmixplus-docs/other/installation/#install-firefox-scripts), or from the following direct links:</p><ul><li>[Configuration files (`fx-folder.zip`)](https://github.com/onemen/TabMixPlus/releases/download/dev-build/fx-folder.zip)</li><li>[Utils (`utils.zip`)](https://github.com/onemen/TabMixPlus/releases/download/dev-build/utils.zip)</li></ul></li><li><p>Open *[about:support](https://support.mozilla.org/kb/use-troubleshooting-information-page-fix-firefox)* in Firefox to identify both the program folder and the profile folder:</p><ul><li>Under *Application Binary*, find the path to `firefox.exe` (e.g., `C:\Program Files\Mozilla Firefox`).</li><li>Under *Profile Folder*, click *Open Folder* to access it.</li></ul></li><li><p>Extract the required files into the correct folders:</p><ul><li>Open `fx-folder.zip` and from `fx-folder`, extract `config.js` and `defaults` directly into the program folder (admin rights may be needed).</li><li>Create the `chrome` folder in the profile folder if not exist.</li><li>Create the `utils` folder in the `chrome` folder if not exist.</li><li>Open `utils.zip`, extract all files directly into the `utils` folder.</li></ul></li><li><p>Click the *Clear startup cache…* button in the top right of *about:support* to restart Firefox.</p></li><li><p>Confirm that the setup is working correctly:</p><ul><li><p>Open *[about:config](https://support.mozilla.org/kb/about-config-editor-firefox)* and search for `userChromeJS.enabled`. Verify that the preference exists without a delete button (🗑️) on the right. If the delete button is present, click it to delete the preference, then restart Firefox and recheck.</p></li></ul></li></ol><p>⛔ DO NOT install *Tab Mix Plus* as it will cause conflict.</p></details>
	- [*MrOtherGuy/fx-autoconfig*](https://github.com/MrOtherGuy/fx-autoconfig) (with update-checking feature) <details><summary>Installation Steps</summary><p>📝 Note: These steps are provided as a simple reference. For complete and accurate instructions, please consult the official page above.</p><ol><li><p>Get the `fx-autoconfig-master.zip` file from the [direct link](https://github.com/MrOtherGuy/fx-autoconfig/archive/refs/heads/master.zip), or from the [official page](https://github.com/MrOtherGuy/fx-autoconfig):</p><ul><li>Click the green *Code* button.</li><li>Select *Download ZIP*.</li></ul></li><li><p>Open *[about:support](https://support.mozilla.org/kb/use-troubleshooting-information-page-fix-firefox)* in Firefox to identify both the program folder and the profile folder:</p><ul><li>Under *Application Binary*, find the path to `firefox.exe` (e.g., `C:\Program Files\Mozilla Firefox`).</li><li>Under *Profile Folder*, click *Open Folder* to access it.</li></ul></li><li><p>Extract `fx-autoconfig-master.zip` and place the required files into the correct folders:</p><ul><li>From `fx-autoconfig-master\program`, place `config.js` and `defaults` directly into the program folder (admin rights may be needed).</li><li>From `fx-autoconfig-master\profile`, place `chrome` directly into the profile folder.</li></ul></li><li><p>If you already have scripts in use:</p><ul><li>Option A: Move all `*.uc.js` files from the `chrome` folder into the `chrome\JS` sub-folder.</li><li>Option B: Edit `chrome\utils\chrome.manifest` and change <code>content userscripts <mark>../JS/</mark></code> to <code>content userscripts <mark>../</mark></code>. This will load scripts directly from the `chrome` folder.</li></ul></li><li><p>Click the *Clear startup cache…* button in the top right of *about:support* to restart Firefox.</p></li><li><p>Confirm that the setup is working correctly:</p><ul><li>Check that a new menu item *userScripts* appears under the *Tools* menu (`Alt`+`T`).</li></ul></li><li><p>Additionally, enable the automatic update checking feature:</p><ul><li>Open *[about:config](https://support.mozilla.org/kb/about-config-editor-firefox)*, paste `userChromeJS.updates.update-check.enabled` into the search box, and click the Add button (✚) to create a boolean preference set to `true`.</li></ul></li></ol></details>
 	- [*alice0775/userChrome.js*](https://github.com/alice0775/userChrome.js) <details><summary>Installation Steps</summary><p>📝 Note: These steps are provided as a simple reference. For complete and accurate instructions, please consult the official page above (Japanese).</p><ol><li><p>Open *[about:support](https://support.mozilla.org/kb/use-troubleshooting-information-page-fix-firefox)* in Firefox to identify both the program folder and the profile folder:</p><ul><li>Under *Application Binary*, find the path to `firefox.exe` (e.g., `C:\Program Files\Mozilla Firefox`).</li><li>Under *Profile Folder*, click *Open Folder* to access it.</li></ul></li><li><p>Go to the [official page](https://github.com/alice0775/userChrome.js), find the *Go to file* search box on the left of the green *Code* button, or simply press the `T` key to focus it:</p><ul><li>Type `userChrome.js` and choose the latest version, typically called `xxx/userChrome.js`. Click the download button to save the file.<br><img src="https://cdn.jsdelivr.net/gh/Merci-chao/userChrome.js@main/screenshots/github-download-file.png"></li><li>Type `install_folder` and choose the latest version, typically called `xxx/install_folder`. Download both `config.js` and `defaults/pref/config-prefs.js`.</li></ul><p>⚠️ Creating the files by copy-paste or other methods may cause incorrect encoding.</p></li><li><p>Place the required files into the program folder:</p><ul><li>Place `config.js` into the program folder (admin rights may be needed).</li><li>Place `config-prefs.js` into the `defaults\pref` sub-folder under the program folder.</li></ul></li><li><p>Place the required file into the profile folder:</p><ul><li>Create the `chrome` folder in the profile folder if not exist.</li><li>Place `userChrome.js` into the `chrome` folder.</li></ul></li><li><p>Click the *Clear startup cache…* button in the top right of *about:support* to restart Firefox.</p></li><li><p>Confirm that the setup is working correctly:</p><ul><li><p>Press `Ctrl`+`Shift`+`J` to open the *Browser Console*. Enable the *Logs* filter, enter `getScripts` in the *Filter Output*, and verify that the related logs appear.</p><img src="https://cdn.jsdelivr.net/gh/Merci-chao/userChrome.js@main/screenshots/alice-scriptloader-en.png"></li></ul></li></ol></details>
 	- [*BSTweaker/UserChromeJS*](https://bitbucket.org/BSTweaker/userchromejs/src/master/loader/) <details><summary>Installation Steps</summary><p>📝 Note: These steps are provided as a simple reference. For complete and accurate instructions, please consult the official page above (Japanese).</p><ol><li><p>Open *[about:support](https://support.mozilla.org/kb/use-troubleshooting-information-page-fix-firefox)* in Firefox to identify both the program folder and the profile folder:</p><ul><li>Under *Application Binary*, find the path to `firefox.exe` (e.g., `C:\Program Files\Mozilla Firefox`).</li><li>Under *Profile Folder*, click *Open Folder* to access it.</li></ul></li><li><p>Prepare the required files:</p><ul><li><p>Go to the [official page](https://bitbucket.org/BSTweaker/userchromejs/src/master/loader/), copy the source code shown there, and save it as a file named `config.js` using UTF-8 encoding without BOM.</p><img src="https://cdn.jsdelivr.net/gh/Merci-chao/userChrome.js@main/screenshots/BSTweaker-config-js.png"></li><li><p>Create a file named `config-prefs.js` with the following content, also saved in UTF-8 encoding without BOM:</p><pre>pref("general.config.obscure_value", 0);<br>pref("general.config.filename", "config.js");<br>pref("general.config.sandbox_enabled", false);</pre></li><li><p>Download [`UserChromeJSLoader.mjs`](https://bitbucket.org/BSTweaker/userchromejs/src/master/loader/UserChromeJSLoader.mjs):</p><img src="https://cdn.jsdelivr.net/gh/Merci-chao/userChrome.js@main/screenshots/BSTweaker-save-file.en.png"></li></ul></li><li><p>Place the required files into the program folder:</p><ul><li>Place `config.js` into the program folder (admin rights may be needed).</li><li>Place `config-prefs.js` into the `defaults\pref` sub-folder under the program folder.</li></ul></li><li><p>Place the required file into the profile folder:</p><ul><li>Create the `chrome` folder in the profile folder if not exist.</li><li>Place `UserChromeJSLoader.mjs` into the `chrome` folder.</li></ul></li><li><p>Click the *Clear startup cache…* button in the top right of *about:support* to restart Firefox.</p></li><li><p>Confirm that the setup is working correctly:</p><ul><li>Check that a new menu item *UserChromeJSLoader* appears under the *Tools* menu (`Alt`+`T`).</li></ul></li></ol></details>
 	- [*Endor8/userChrome.js*](https://github.com/Endor8/userChrome.js) <details><summary>Installation Steps</summary><p>📝 Note: These steps are provided as a simple reference. For complete and accurate instructions, please consult the official page above (German).</p><ol><li><p>Open *[about:support](https://support.mozilla.org/kb/use-troubleshooting-information-page-fix-firefox)* in Firefox to identify both the program folder and the profile folder:</p><ul><li>Under *Application Binary*, find the path to `firefox.exe` (e.g., `C:\Program Files\Mozilla Firefox`).</li><li>Under *Profile Folder*, click *Open Folder* to access it.</li></ul></li><li><p>Download the [`userChrome.js`](https://github.com/Endor8/userChrome.js/blob/master/userChrome.js) file through the download button:</p><img src="https://cdn.jsdelivr.net/gh/Merci-chao/userChrome.js@main/screenshots/github-download-file.png"><p>⚠️ Creating the files by copy-paste or other methods may cause incorrect encoding.</p></li><li><p>Go to the [official page](https://github.com/Endor8/userChrome.js), find and download the latest version of the following files:</p><ul><li>`config.js`</li><li>`config-prefs.js`</li><li>`userChromeJS\main.js`</li><li>`userChromeJS\utilities.js`</li></ul></li><li><p>Place the required files into the program folder:</p><ul><li>Place `config.js` into the program folder (admin rights may be needed).</li><li>Place `config-prefs.js` into the `defaults\pref` sub-folder under the program folder.</li><li>Create the `userChromeJS` folder in the program folder if not exist.</li><li>Place `main.js` and `utilities.js` into the `userChromeJS` folder.</li></ul></li><li><p>Place the required file into the profile folder:</p><ul><li>Create the `chrome` folder in the profile folder if not exist.</li><li>Place `userChrome.js` into the `chrome` folder.</li></ul></li><li><p>Click the *Clear startup cache…* button in the top right of *about:support* to restart Firefox.</p></li></ol></details>
   <p>There is no difference among them for applying this script, so simply choose the one you prefer.</p>
...
6. If you moved some files out of the `chrome` folder in step 3, please confirm that this script works as expected. Once verified, you can move the files back. If any conflicts arise, modify the scripts and the style rules in `userChrome.css` accordingly. If you need help, please provide the information 🛟 [here](https://github.com/Merci-chao/userChrome.js/issues/new).
...
| ------------- | ------------- |
| `animateTabMoveShiftKeyToPause` | <p>🔸 **Pause Drag Animation with Shift Key**</p><p>When pressing `Shift` key, pause the drag & drop animation and show the drop indicator instead. This feature can be helpful in special situations where it is difficult to move items next to a specific one in another row.</p> |
| `animateTabMoveUnderLimit` | <p>🔸 **Drag Animation Limit**</p><p>When the number of visible tabs reaches this amount, disable the drag & drop animation and show the drop indicator instead. If dragging becomes sluggish with many tabs, consider lowering this value. If you prefer to always use the indicator for moving tabs, set the value below `3` to effectively force indicator mode.</p> |
| `animationDuration` | <p>🔸 **Animation Duration**</p><p>Duration of animations in milliseconds, valid range: `0` - `1000`. Setting the value to `0` will disable all tab‑related animations.</p><p>📝 Note: Lengthy animations could strain system performance.</p> |
| `dragStackPreceding` | <p>🔸 **Stack Preceding Tabs on Drag**</p><p>Stack the preceding selected tabs of the dragged one (see [`browser.tabs.dragDrop.multiselectStacking`](#multiselectStacking)). When dragging the middle tab among selected ones, the following ones of the selected tabs may move forward undesirably. Disabling this setting can avoid the issue.</p> |
| `dynamicMoveOverThreshold` | <p>🔸 **Smooth Dragging**</p><p>Make tab-dragging movement smoother in certain scenarios, e.g. dragging pinned or grouped tabs. Not available on Firefox 115, or `browser.tabs.dragDrop.createGroup.enabled` is `false`.</p> |
| `hideDragPreview` | <p>🔸 **Hide Drag Preview**</p><p>Hide the drag preview that appears next to the cursor during dragging:</p><ul><li>`0` - never</li><li>`1` - tab groups only</li><li>`2` - tabs only</li><li>`3` - both</li></ul> |
| `hidePinnedDropIndicator` | <p id="hidePinnedDropIndicator">🔸 **Hide Pinned Drop Indicator**</p><p>Hide the indicator that appears when dragging a tab to pin it, if there are no existing pinned tabs. Not available on Firefox 115 and 140.</p> |
| `hideScrollButtonsWhenDragging` | <p>🔸 **Hide Scroll Buttons on Drag**</p><p>Visually hide the up/down scroll buttons when dragging.</p> |
| `linesToDragScroll` | <p>🔸 **Drag Scroll Rows**</p><p>How many rows to scroll when dragging tabs to top/bottom edge. Minimum: `1`.</p> |
| `linesToScroll` | <p>🔸 **Wheel Scroll Rows**</p><p>How many rows to scroll when using the mouse wheel. Minimum: `1`.</p> |
| `previewPanelNoteEditable` | <p>🔸 **Editable Notes in Preview Panel**</p><p>Allow the tab preview panel to be hovered and the note inside to be editable when the tab note feature of Firefox is enabled. Not available on Firefox 115 and 140.</p> |
| `previewPanelShifted` | <p>🔸 **Shift Preview Panel**</p><p>Shift the preview panel when there are multiple rows, reducing the effect of the panel blocking items in the rows underneath. Affects tabs only when `previewPanelNoteEditable` is `true`. Not available on Firefox 115 and 140.</p><ul><li>`0` - never</li><li>`1` - for groups</li><li>`2` - for tabs</li><li>`3` - for both</li></ul> |
| `previewPanelShiftedAlways` | <p>🔸 **Always Shift Preview Panel**</p><p>Shift the preview panel even when there is only one row.</p> |
| `scrollButtonsSize` | <p>🔸 **Scroll Button Height**</p><p>The height (in pixels) of the scroll buttons during dragging. Minimum: `0`, but it will be rendered as at least 2 device pixels in height; the maximum is limited to half the tab height.</p> |
...
| `privateBrowsingIconOnNavBar` | <p>🔸 **Private Browsing Icon on Navigation Toolbar**</p><p>Move the private window icon to the navigation toolbar. Not available on Firefox 115. Forcibly activated when `tabsAtBottom` is enabled.</p> |
| `rowIncreaseEvery` | <p>🔸 **Row Increase per Width**</p><p>Each time the window width increases by this amount, one more row is allowed to be displayed. A narrower window will therefore show fewer rows at once. When set to the minimum value `0`, the maximum number of rows is directly allowed to be displayed.</p> |
| `rowStartIncreaseFrom` | <p>🔸 **Row Increase Start Width**</p><p>When the window width is larger than this number plus `rowIncreaseEvery`, multi-row display is allowed.</p> |
| `smartWindowButtonOnNavBar` | <p>🔸 **Smart Window Button on Navigation Toolbar**</p><p>On classic windows, move the Firefox Smart Window switching button to the navigation toolbar. Forcibly activated when `tabsAtBottom` is enabled. Available on Firefox 149 to 153. Directly move the button via *Customize Toolbar* on Firefox 154+.</p> |
| `spaceAfterTabs` | <p>🔸 **Top-Right Space**</p><p>Empty space before the window control buttons. Minimum: `0`. Available when the title bar is hidden. Affects only web apps when the menu bar is displayed.</p> |
| `spaceAfterTabsOnMaximizedWindow` | <p>🔸 **Top-Right Space (Maximized)**</p><p>Empty space before the window control buttons, when maximized. Minimum: `0`. Available when the title bar is hidden. Affects only web apps when the menu bar is displayed.</p> |
...
| `spaceBeforeTabsOnMaximizedWindow` | <p>🔸 **Top-Left Space (Maximized)**</p><p>Empty space on the top left corner of the window, when maximized. Minimum: `0`. Available when the title bar is hidden. Affects only web apps when the menu bar is displayed.</p> |
...
| `tabsUnderControlButtons` | <p>🔸 **Full-Width Tab Bar Usage**</p><p>Tabs starting from the second row use the full width of the tab bar:</p><ul><li>`0` - never</li><li>`1` - when the tab bar is not scrollable (legacy option, not recommended)</li><li>`2` - always</li></ul> |
...
| `tabHorizontalMargin` | <p>🔸 **Tab Horizontal Margin**</p><p>Horizontal space around tab. Minimum: `0`.</p> |
| `tabHorizontalPadding` | <p>🔸 **Tab Horizontal Padding**</p><p>Horizontal padding of tab. Minimum: `0`.</p> |
| `tabHorizontalPaddingCompact` | <p>🔸 **Tab Horizontal Padding in Auto-Compact**</p><p>Compact mode size when Nova automatic window density is enabled in Firefox 154+. The value is capped to the standard mode value.</p> |
| `tabMaxWidth` | <p>🔸 **Tab Maximum Width**</p><p>Maximum width of tabs, including the surrounding white space. Use `browser.tabs.tabMinWidth` for the minimum width, and the actual maximum width will never be lower than that.</p> |
| `tabVerticalMargin` | <p>🔸 **Tab Vertical Margin**</p><p>Vertical space around tab. Minimum: `0`.</p> |
| `tabVerticalMarginCompact` | <p>🔸 **Tab Vertical Margin in Auto-Compact**</p><p>Compact mode size when Nova automatic window density is enabled in Firefox 154+. The value is capped to the standard mode value.</p> |
...
| `browser.tabs.tabClipWidth` | <p>🔸 **Close Button Display Width**</p><p>Close buttons will show on the tabs that are wider than this size. Takes effect in new windows after changing. When the value is:</p><ul><li>equal to or larger than `tabMaxWidth` - never show close buttons on non-selected tabs</li><li>smaller than `browser.tabs.tabMinWidth` - always show close buttons on non-selected tabs</li></ul> |
...
1. Ensure you are using the latest version of Multi Tab Rows:
...
2. Ensure you are using the latest version of Firefox:
...
3. Ensure you are using the latest version of script loader:
