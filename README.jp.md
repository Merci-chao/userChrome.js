...
- このスクリプトは小細工というより、包括的なタブ拡張に近い。数千行に及ぶロジックとスタイルをひとつのファイルにまとめており、典型的な `userChrome.js` スクリプトと同じくらい扱いやすいままになっている。完璧さとパフォーマンスを意識したハードコアな実装であり、行数は問題にならない――[Tab Mix Plus](https://onemen.github.io/tabmixplus-docs) が数百行だけで実現できることは決してないのと同じ。ただし、もしシンプルで理解しやすく、十分な安心感を感じられるものを好むなら、これは好ましい選択にはならないかもしれない。
