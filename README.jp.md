...
- このスクリプトは単純な調整というより、包括的なタブ拡張に近い。数千行に及ぶロジックとスタイルをひとつのファイルにまとめており、典型的な `userChrome.js` スクリプトと同じくらい扱いやすいままになっている。完璧さとパフォーマンスを意識したハードコアな実装であり、行数は問題にならない――[Tab Mix Plus](https://onemen.github.io/tabmixplus-docs) が数百行だけで実現できることは決してないのと同じ。ただし、もしコードがシンプルで完全に理解しやすく、より強い安心感を与えるものを好むなら、これは好ましい選択にはならないかもしれない。
