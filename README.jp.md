...
- 一般的なスクリプトローダーに対応。例：
	...
	- [`fx-autoconfig`](https://github.com/MrOtherGuy/fx-autoconfig)
 	- ~~[`userChrome.js`](https://github.com/alice0775/userChrome.js)~~（旧バージョンのみ）
...
1. スクリプトローダーをインストールする。すでにスクリプトを使用している場合、このステップは省略可能。使えるスクリプトローダーはいくつかある：
	...
	- ~~[`userChrome.js`](https://github.com/alice0775/userChrome.js)（作者：[alice0775](https://github.com/alice0775)）~~（最新バージョンでは `security.allow_unsafe_dangerous_privileged_evil_eval` がロックされ、それに依存するスクリプトが失敗する。）
