...
<table>
  <tr>
    <td>🚀&nbsp;最新‍機‍能‍サ‍ポ‍ー‍ト</td>
    <td>タブグループ、分割ビュー、タブスタッキングなど、最新のタブ機能を完全にサポート。</td>
  </tr>
  <tr>
    <td>🖱️&nbsp;多段ドラッグ＆ドロップ</td>
    <td>多段でも、アニメーション付きドラッグ＆ドロップを唯一サポート。</td>
  </tr>
  <tr>
    <td>✨&nbsp;スムーズなユーザー体験</td>
    <td>タブ操作において、シームレスで洗練された体験を提供。</td>
  </tr>
  <tr>
...
- このスクリプトは単純な調整というより、包括的なタブ拡張に近い。一万程度の行に及ぶロジックとスタイルをひとつのファイルにまとめており、典型的な `userChrome.js` スクリプトと同じくらい扱いやすいままになっている。完璧さとパフォーマンスを意識した本格的な実装であり、開発では行数は気にする点ではない――[Tab Mix Plus](https://onemen.github.io/tabmixplus-docs) が数百行だけで実現できることは決してないのと同じ。ただし、もしコードがシンプルで完全に理解しやすく、より強い安心感を与えるものを好むなら、これは好ましい選択にはならないかもしれない。
- Firefox に最適化されたレイアウト計算を多く含むため、タブ関連の旧式拡張（例：Tab Mix Plus）、スクリプト（`*.uc.js`）、CSS スタイル（`userChrome.css`）との競合によって、不具合が発生する可能性がある。<br>⚠️ 以下のインストール手順をよく読んで、慎重に進めてください。
...
| `floatingBackdropBlurriness` | タブバーがスクロール可能時にタブを覆う要素の背景ぼかし強度を設定する。`tabsUnderControlButtons` が `2` かつ `floatingBackdropClip`、`nativeWindowStyle` が `false` のときのみ有効。Firefox 115 では非対応。 |
