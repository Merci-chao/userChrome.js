...
- このスクリプトは Windows 向けに作られ、Ubuntu（Linux）で簡単なテストをしている。macOS では試していないので、動かないか不具合が出る可能性がある。Firefox をベースとしたその他の派生ブラウザーはサポート対象外。Firefox をベースとしたその他の派生ブラウザーはサポート対象外。
