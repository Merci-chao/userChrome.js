...
1. スクリプトローダーをインストールする。すでにスクリプトを使用している場合、このステップは省略可能。使えるスクリプトローダーはいくつかある：
	...
	- [`userChrome.js`](https://github.com/alice0775/userChrome.js)（作者：[alice0775](https://github.com/alice0775)）（制限が存在する場合、`config.js` 内の `security.allow_unsafe_dangerous_privileged_evil_eval` のロックされた値を `true` に設定する必要がある）
