...
/* global
   gBrowser, RTL_UI, Services, Cu, Cc, Ci, promiseDocumentFlushed, XPCOMUtils,
   gURLBar, gNavToolbox, gReduceMotion, FullScreen, TAB_DROP_TYPE, InspectorUtils, windowUtils,
   gNotificationBox, gTabsPanel, AIWindow, fullScreen, mozInnerScreenX, gUIDensity, TabBarVisibility,
*/

if (document.documentElement.matches(`[windowtype="navigator:browser"]:not([popup-window], [mini-window], [chromehidden~=toolbar])`)) {
	...
	if (!getPref(prefBranchStr + name)) {
		...
		if (compatible)
			...
			if (getPref(compatibilityPref) != prefValue) {
				let [suggestedVer, maxSupportVer] = compatibility.at(-1);
				maxSupportVer ||= suggestedVer;

				let texts = l10n({
					en: {
						msg1: `You are currently using Firefox ${appVersion}, but this script only supports Firefox ${compatibility.map(r => r.join("–")).join(", ")}.`,
						msg2:
							maxSupportVer < appVersion
								? `To ensure compatibility, please reset “checkUpdate” to 1 to enable update checking, or manually update this script to the latest version.`
								: `To ensure compatibility, please update Firefox to ${suggestedVer} or above.`,
						checkMsg: "Don't show again",
					},
					ja: {
						msg1: `現在使用しているバージョンは Firefox ${appVersion} ですが、このスクリプトは Firefox ${compatibility.map(r => r.join("～")).join("、")} のみをサポートしています。`,
						msg2:
							maxSupportVer < appVersion
								? `互換性を確保するため、更新チェックを有効にするには「checkUpdate」を 1 にリセットするか、スクリプトを手動で最新バージョンに更新してください。`
								: `互換性を確保するため、Firefox を ${suggestedVer} 以上に更新してください。`,
						checkMsg: "今後表示しない",
					},
				});
...
let closingTheOnlyTab = false;
let removingFirefoxViewTab = false;
...
const nativeThemeMQ = matchMedia("(-moz-native-theme)");
...
//should be selectors directly under #mainPopupSet and splittable by comma
const TABS_RELATED_PANELS = `
	#tab-preview-panel,
	#tabgroup-preview-panel,
	#tab-group-editor > .tab-group-editor-panel,
	#tab-note-menu > #tabNotePanel,
	#tab-note-preview-panel
`;

const HAS_DRAG_TO_PIN = !!window.TabDragAndDrop;
const HAS_TAB_GROUP = "tabGroups" in gBrowser;
const HAS_SPLIT_VIEW = "addTabSplitView" in gBrowser;
const HAS_SWITCH_BY_SCROLLING_PROP = "switchByScrolling" in tabContainer;
const HAS_TAB_NOTE = "tabNoteMenu" in gBrowser;
const HAS_TAB_GROUP_PREVIEW = appVersion > 143;
const HAS_TAB_STACKING = appVersion > 148 || "TabStacking" in window;
const HAS_AI_WINDOW = appVersion > 148;
const HAS_AUTO_UI_DENSITY = appVersion > 153;
const HAS_NOVA = appVersion > 151;
const AI_SWITCHER_FIXED = appVersion < 154;
const BACKGROUND_ON_BODY = appVersion > 149;
const NO_POST_TABS_SPACE = appVersion > 154;
const URLBAR_POPOVER = appVersion < 157;

const FOR_GROUP = 1;
const FOR_TAB = 2;

const __TAB_MARGIN_OVERFLOW_CLIP =
	appVersion > 157 ? "--tab-margin-overflow-clip" : "--tab-overflow-clip-margin";
const __TAB_MARGIN_BLOCK =
	appVersion > 153 ? "--tab-margin-block" : "--tab-block-margin";
const __TAB_PADDING_INLINE =
	appVersion > 157
		? "--tab-padding-inline"
		: appVersion > 131 ? "--tab-inline-padding" : "--inline-tab-padding";
...
const TAB_INLINE_PADDING = [8, HAS_AUTO_UI_DENSITY ? 6 : 8, 8];
...
if (NO_POST_TABS_SPACE) {
	let box = $(".titlebar-spacer[type=pre-tabs]", tabsBar).cloneNode();
	box.setAttribute("type", "post-tabs");
	$(".toolbar-items", tabsBar).after(box);
}

/** @type {Object} */
let prefs;
function createDefaultPrefs() {
	const allowTransparent = appVersion > 130 || win7 || win8;
	const shouldClip =
	...
	return {
		...
		spaceAfterTabs: NO_POST_TABS_SPACE ? 0 : 40,
		spaceAfterTabsOnMaximizedWindow: NO_POST_TABS_SPACE ? 0 : 40,
		spaceBeforeTabs: 40,
		...
		floatingBackdropOpacity: appVersion < 117 || shouldClip ? 100 : 50,
		...
		dynamicMoveOverThreshold: HAS_TAB_GROUP ? true : null,
		nativeWindowStyle: allowTransparent ? false : null,
		toolbarColorOpacity:
			allowTransparent
				? !nova && getPref(prefBranchStr + "tabsAtBottom") ? 0 : 100
				: null,
		urlBarColorOpacity: allowTransparent ? 100 : null,
		themeGradientOpacity: HAS_NOVA ? 100 : null,
		...
		privateBrowsingIconOnNavBar:
			$("#private-browsing-indicator-with-label") ? null : false,
		...
		tabVerticalMarginCompact: HAS_AUTO_UI_DENSITY ? TAB_BLOCK_MARGIN_NOVA[1] : null,
		tabHorizontalPadding: (nova ? TAB_INLINE_PADDING_NOVA : TAB_INLINE_PADDING)[uiDensity],
		tabHorizontalPaddingCompact: HAS_AUTO_UI_DENSITY ? TAB_INLINE_PADDING_NOVA[1] : null,
		tabHorizontalMargin: TAB_INLINE_MARGIN,
		tabCornerRadius: -1,
		previewPanelNoteEditable: HAS_TAB_NOTE ? true : null,
		previewPanelShifted: HAS_TAB_GROUP_PREVIEW ? FOR_TAB | FOR_GROUP : null,
		previewPanelShiftedAlways: HAS_TAB_GROUP_PREVIEW ? false : null,
		newTabButtonAfterLastTab: true,
		positionPinnedTabs: true,
		lastRowTabsFlexible: true,
		animateTabMoveShiftKeyToPause: true,
		// dlpButtonOnNavBar: $(".content-analysis-indicator") ? false : null,
		smartWindowButtonOnNavBar: AI_SWITCHER_FIXED && HAS_AI_WINDOW ? false : null,
...
nativeThemeMQ.onchange = () => onPrefChange(null, null, "browser.theme.native-theme");
colorSchemeMQ.onchange = () =>
	prefs.themeGradientOpacity < 100 &&
	onPrefChange(null, null, "browser.theme.toolbar-theme");
...
function loadPrefs(defaultPrefs = createDefaultPrefs()) {
	...
	clamp({
		...
		toolbarColorOpacity: [0, 100],
		urlBarColorOpacity: [0, 100],
		themeGradientOpacity: [0, 100],
		tabContentHeight: [16, TAB_CONTENT_HEIGHT[0] * 2],
		tabVerticalMargin: [0, TAB_BLOCK_MARGIN * 3],
		tabHorizontalPadding: [0, TAB_INLINE_PADDING[0] * 3],
		tabHorizontalMargin: [0, TAB_INLINE_MARGIN * 5],
	});

	clamp({
		tabContentHeightCompact: [16, prefs.tabContentHeight],
	...
	const supportsHas = getPref("layout.css.has-selector.enabled", true, true);
	...
	lock("themeGradientOpacity", !cssImgTheme);
	lock(["compactControlButtons", "controlButtonsAutoHide"], allWindowsHaveTitleOrMenuBar);
	lock(
		["controlButtonsAutoHideTriggerHeight", "controlButtonsAutoHideOnTriggerExit"],
		!prefs.controlButtonsAutoHide,
	);
	...
	lock(
		...
		(
			alwaysIndMove ||
			!getPref("browser.tabs.dragDrop.createGroup.enabled", HAS_TAB_GROUP) ||
	...
	lock("hidePinnedDropIndicator", !getPref("browser.tabs.dragDrop.dragToPin.enabled", HAS_DRAG_TO_PIN) || alwaysIndMove, true);
	lock("pinnedTabsFlexWidthIndicator", !prefs.pinnedTabsFlexWidth);
	lock("dragStackPreceding", !getPref("browser.tabs.dragDrop.multiselectStacking", HAS_TAB_STACKING) || alwaysIndMove);
	lock(
	...
	lock("hideEmptyPlaceholderWhenScrolling", prefs.tabsUnderControlButtons < 2);
	lock("previewPanelNoteEditable", !getPref("browser.tabs.notes.enabled", HAS_TAB_NOTE, true), false);
	lock("positionPinnedTabs", prefs.pinnedTabsFlexWidth || autoCollapse, false);
	lock("lastRowTabsFlexible", prefs.justifyCenter > 1);
	...
	if (showMenubar && root.matches(":not([popup-window], [taskbartab], [chromehidden~=menubar])"))
...
async function onPrefChange(pref, type, name) {
	...
	switch (name) {
		...
		case "browser.theme.native-theme":
		case "browser.theme.windows.accent-color-in-tabs.enabled":
	...
	if (name == "browser.nova.enabled")
		queueMicrotask(() => tabContainer[ON_UI_DENSITY_CHANGED]());
...
const SCRIPT_START_DATE = new Date().setHours(0, 0, 0, 0) / 1000;
if (
	prefs.checkUpdate && (SCRIPT_START_DATE - prefs.checkUpdate) / 60 / 60 / 24
		>= Math.max(prefs.checkUpdateFrequency, 1)
) {
	Services.prefs.setIntPref(prefBranchStr + "checkUpdate", SCRIPT_START_DATE);
	(async () => {
		...
		function setTomorrow() {
			Services.prefs.setIntPref(
				...
				SCRIPT_START_DATE - (Math.max(prefs.checkUpdateFrequency, 1) - 1) * 24 * 60 * 60,
			);
		}
	})();
}
...
let mainStyle = assign(document.body.appendChild(document.createElement("style")), {id: "multiTabRows-mainStyle"});
...
const __TAB_ICON_MARGIN_INLINE_END =
	appVersion > 157 ? "--tab-icon-margin-inline-end" : "--tab-icon-end-margin";
...
const notNovaPrivate = nova && defaultTheme && !prefs.nativeWindowStyle
	? `:not([privatebrowsingmode=temporary])` : ``;
const roundConentCanDisable = Services.prefs.prefHasDefaultValue("sidebar.revamp.round-content-area");
const topPlaceholdersWithSidebarAbove = `
...
const aiHideNavbar =
	appVersion > 155
		? "[aiwindow-first-run]"
		: appVersion > 152
			? ":is([aiwindow-new-window], [aiwindow-first-run])"
			: "[aiwindow-immersive-view]";
...
const borderSnapping = size => CSS.supports("flex", "round(1)")
	...
	? size == "1px" && appVersion > 140
		? `var(--snapped-border-width)`
		: /*js*/`calc(
...
const outlineOffsetSnappingPref = [1, 2].includes(getPref("layout.css.outline-offset.snapping", 0, true));
...
const layeredBackgroundSytle = `
	...
	background-image:
		...
		linear-gradient(var(--chrome-block-toolbar-color)),
		linear-gradient(var(--chrome-block-foreground-color)),
		var(--chrome-block-background-image),
		linear-gradient(var(--chrome-block-background-color));
	background-color: var(--multirows-toolbox-background-color);
	background-repeat:
		repeat,
		repeat,
...
const toolbarBackgroundStyle = nova
	? layeredBackgroundSytle : themeBackgroundStyle;

let css = /*css*/`
:root {
	--max-tab-rows: 1;
	--snapped-border-width: env(hairline, 1px / var(--device-pixel-ratio));
	...
	` : ``}
	${__TAB_PADDING_INLINE != "--tab-padding-inline" ? `
		--tab-padding-inline: var(--tab-inline-padding, var(--inline-tab-padding));
	` : ``}
	--tab-margin-overflow-clip: ${prefs.tabHorizontalMargin}px;
	--tab-overflow-clip-margin: var(--tab-margin-overflow-clip);
	...
	` : ``}
	--tab-pinned-inline-padding: ${HAS_NOVA
		? `calc((var(--tab-min-height) - var(--icon-size)) / 2 - var(--tab-padding-inline))`
		: "2px"};
	...
	${__TAB_ICON_MARGIN_INLINE_END}: 6px;
	...
	--nav-toolbox-margin-top: 0px;
	--nav-toolbox-margin-bottom: 0px;
	--nav-toolbox-margin-inline: 0px;
	--nav-toolbox-border-top: 0px;
	--nav-toolbox-border-bottom: 0px;
				...
				? `max(
					var(--nav-toolbox-net-padding-box-height)
						${!isYAlign ? `
							...
							+ var(--nav-toolbox-margin-bottom)
							+ var(--nav-toolbox-border-top)
							+ var(--nav-toolbox-border-bottom)
						` : ""}
					...
					${isYAlign ? `
						var(--nav-toolbox-border-box-height, 0px)
					` : `
						var(--browser-content-top, 0px)
					`}
				)`
				: "0px"
		};
	...
	@media -moz-pref("widget.windows.mica.toplevel-backdrop", 2) {
		--mica-background-color:
			${prefs.nativeWindowStyle
				? "#D3D3D3"
				: `color-mix(in srgb, white 60%, #D3D3D3)`};
		@media (prefers-color-scheme: dark) {
			--mica-background-color:
				${prefs.nativeWindowStyle ?
					"#545454"
					: `color-mix(in srgb, black 60%, #545454)`};
		}
	}
}
...
@media (resolution >= 2dppx) {
	:root {
		--snapped-border-width: calc(env(hairline, 1px / var(--device-pixel-ratio)) * 2);
	}
}

${!nova ? /*css*/`
	:root[uidensity=compact] {
		--tab-pinned-inline-padding: 0px;
	}
` : ``}
...
body {
	--toolbox-background-color-current: var(--toolbox-background-color, var(--toolbox-bgcolor));
}

body:-moz-window-inactive {
	--toolbox-background-color-current: var(--toolbox-background-color-inactive, var(--toolbox-bgcolor-inactive));
}
...
${cssImgTheme ? /*css*/`
	:root[theme-image-in-toolbox]:not([ai-window])${notNovaPrivate} {
		--multirows-background-size:
			calc(100vw - var(--nav-toolbox-margin-inline) * 2)
			var(--nav-toolbox-border-box-height);
		--multirows-background-position:
...
${_="#TabsToolbar"} {
	...
	--tabs-placeholder-border-color: ${placeholderBorderColor};
...
${!prefs.tabsAtBottom ? /*css*/`
	${_}:-moz-window-inactive {
		--tabs-placeholder-border-color:
			color-mix(
				in srgb,
				${placeholderBorderColor} calc(100% * var(--inactive-titlebar-opacity, 1)),
				transparent
			);
	}
` : ``}
...
${prefs.tabCornerRadius > -1 ? /*css*/`
	:is(#tabs-newtab-button, ${adjacentNewTab}) .toolbarbutton-icon {
...
${prefs.privateBrowsingIconOnNavBar ? /*css*/`
	${_} :is(.private-browsing-indicator-with-label, .private-browsing-indicator-button),
	#nav-bar .private-browsing-indicator-label {
		display: none;
	}

	:root[privatebrowsingmode=temporary] #nav-bar#nav-bar
		${nova ? ".private-browsing-indicator-button" : ".private-browsing-indicator-with-label"}
	{
		display: flex;
		${!nova ? `
			margin-inline-end: ${HAS_NOVA ? "var(--space-medium)" : "12px"};
		` : ``}
	}
` : ``}
...
${prefs.controlButtonsAutoHide ? /*css*/`
	...
	${context} ${__}:is(
		...
		#navigator-toolbox[tabs-dragging] *
	) {
	...
	` : /*css*/`
		${context} #toolbar-menubar .titlebar-buttonbox-container {
			min-height: ${prefs.tabsAtBottom
				? `calc(var(--urlbar-height, var(--urlbar-min-height)) + 2 * var(--urlbar-padding-block))`
				: `var(--tabstrip-min-height)`};
		}

		${context} #nav-bar .titlebar-buttonbox-container {
			min-height: calc(var(--urlbar-height, var(--urlbar-min-height)) + 2 * var(--urlbar-padding-block));
		}
	`}
` : ``}
...
#TabsToolbar-customization-target > * {
	align-self: ${prefs.tabsbarItemsAlign} !important;
}
...
${_="#tabbrowser-tabs[orient]"} {
	...
	--group-line-padding: ${HAS_TAB_GROUP ? "3px" : "0px"};
	--group-label-max-width: ${HAS_TAB_GROUP ? "10em" : "0px"};
	--group-last-tab-line-indent:
		${nova
			? "calc(var(--tab-margin-overflow-clip) * -1)"
...
${!prefs.autoCollapse ? /*css*/`
	${_}:is([movingtab], [movingtab-finishing]) {
		mask:
			linear-gradient(
...
${prefs.autoCollapse && !taskBarTab ? /*css*/`
	...
	${context=`:root:not([tab-scroll-rows="1"])`} ${_}:not(${TEMP_SHOW_CONDITIONS}) {
		height: var(--tabstrip-min-height);
	}
	...
	${context} ${_}${TEMP_SHOW_CONDITIONS} {
		...
		border-radius:
			${nova
				? `calc(var(--panel-border-radius) / 2)`
				: `var(--panel-border-radius, var(--toolbarbutton-border-radius))`};
...
:is(
	#TabsToolbar[customizing][draggingover],
	#navigator-toolbox:has(> [customizing][draggingover])
)
	${_}
{
	pointer-events: none;
...
@media ${win7 || win8 || prefs.tabsUnderControlButtons < 2 || !OVERLAY_SCROLLBARS && !nova
	...
	*/
	${context = `#TabsToolbar[movingtab] #tabbrowser-tabs:is(
		...
		[moving-single-tab]${HAS_TAB_GROUP ? "[moving-tabgroup]" : ""},
		[movingtab-finishing]
	)`}
		${_}::part(overflow-start-indicator),
...
${prefs.autoCollapse ? /*css*/`
	#tabbrowser-tabs:is(:not([temp-open], :-moz-window-inactive), [animating-collapse]) ${_}::part(scrollbox) {
...
${"#tabbrowser-arrowscrollbox".repeat(3)} tab-group {
	...
	tab-split-view-wrapper {
		...
		&.tab-stack {
			--wrapper-padding: var(--tab-margin-overflow-clip);
		...
		tab-group:not([stacked]) & {
			...
			tab:not(
				tab-group[collapsed] > ${HAS_TAB_GROUP_PREVIEW ? ":not([selected])" : "tab"},
				[stacking]
			) > &::before {
				content: "";
				pointer-events: none;
	...
	& > :is(.tab-group-label-container, tab, tab-split-view-wrapper):not(
		...
		tab-group > tab&,
		tab-split-view-wrapper& {
			--line-indent: calc(var(--group-last-tab-line-indent) + var(--tab-margin-overflow-clip));
		}
	}
...
${appVersion < 134 ? /*css*/`
	${_}[image] > .tab-stack > .tab-content:is([attention], [titlechanged]):not([pinned], [selected]) {
		background-position-x: ${START} calc(var(--tab-padding-inline) + 6px);
	}
` : ``}
...
`}
{
	...
	` : appVersion < 145 ? `
		inset-block-start:
			calc(
				clamp(
					${nova ? "var(--snapped-border-width)" : "0px"},
...
` : nova && prefs.tabCornerRadius < 0 ? /*css*/`
	${!prefs.pinnedTabsFlexWidth ? `tab:not([pinned])` : ``} :is(${context}) {
		--tab-context-line-inline-offset: calc(var(--tab-padding-inline) + 8px);
	}
` : /*css*/`
	${context} {
...
${prefs.pinnedTabsFlexWidth ? /*css*/`
	...
	{
		margin-inline-end: var(${__TAB_ICON_MARGIN_INLINE_END});
...
${_}${condition}::before,
${_}${condition}::after {
	width: var(--tab-margin-overflow-clip);
}
...
.tab-content {
	...
	tab-split-view-wrapper & {
		--tab-overlay-icon-distance-adjustment: min(var(--split-view-padding-block), 1px);
	}
		...
		&[pinned] {
			...
			--tab-overlay-icon-size-adjustment:
				min(
					${nova
						? `var(--tab-min-height) - var(--tab-icon-size) * 2`
						: `var(--tab-padding-inline) - var(--tab-icon-size) / 2`},
					0px
				);
		}
	` : ``}
...
${!prefs.pinnedTabsFlexWidth ? /*css*/`
	:root[id] .tab-content[pinned] {
		padding-inline: calc(var(--tab-padding-inline) + var(--tab-pinned-inline-padding));
	}
` : ``}
...
.tab-close-button {
	padding-block: 0;
	object-fit: scale-down;
	height: min(var(--tab-content-button-size, 24px), var(--tab-min-height));
}
...
.tab-audio-button::part(button) {
	--size: min(var(--tab-content-button-size, 24px), var(--tab-min-height));
...
${"#tabbrowser-arrowscrollbox".repeat(3)} tab-split-view-wrapper {
	...
	--split-view-tab-height: max(calc(var(--tab-min-height) - var(--split-view-padding-block) * 2), var(--tab-icon-size));
	display: flex;
	align-items: center;
	position: relative;
	min-width: var(--splitview-min-width);
	max-width: var(--splitview-max-width);
	outline: 0;
	border-radius: 0;
	border: 0;
	padding: 0;
	...
	&::after {
		...
		background:
			var(--splitview-background-color)
			/*don't use image() as it may not be pixel perfect*/
			linear-gradient(var(--splitview-separator-color))
			center / var(--snapped-border-width) var(--split-view-tab-height)
			no-repeat;
		border-radius: var(--tab-border-radius);
		width: calc(100% - var(--tab-margin-overflow-clip) * 2 + var(--animate-width));
		...
		margin-inline:
			calc(-100% - var(--animate-width) + var(--tab-margin-overflow-clip))
			calc(0px - var(--animate-width) + var(--tab-margin-overflow-clip) ${__});
	}
	...
	&:where(
		:not(
			[hasactivetab],
			[collapsed] > :not([hasactivetab]),
			[closing],
			:has(> [multiselected])
		):has(> :not([closing]) + :not([closing]))
	) {
	...
	&:not([hasactivetab]) {
		--splitview-outline-color: var(--tab-border-color, var(--tab-outline-color));
		...
		&:hover {
			--splitview-background-color: var(--tab-background-color-hover, var(--tab-hover-background-color));
			--splitview-outline-color: var(--tab-border-color-hover, var(--tab-hover-outline-color));
		}
	}
		...
		#tabbrowser-tabs[movingtab-group] & {
			...
			&[dragover-groupTarget] {
				${nova ? `
					...
					--splitview-outline-offset: calc(var(--snapped-border-width) * -1);
				` : `
	...
	tab {
		--tab-margin-overflow-clip: inherit;
		...
		& + & {
			&::before {
				width: calc(var(--tab-margin-overflow-clip) + 1px);
			}
			...
			&[closing] .tab-stack {
				...
				& > * {
					translate: calc(var(--tab-margin-overflow-clip) * -3 * ${DIR}) 0;
		...
		.tab-background {
			--tab-min-height: var(--split-view-tab-height);
...
#tabbrowser-tabs[orient] ${_}[fadein]:is(
	...
	&:not(${__ = "[image], [crashed], [sharing], [pictureinpicture], [busy], [pendingicon]"}) {
		...
		&[tab-note]:is(${showAudioButton}) {
			${__TAB_ICON_MARGIN_INLINE_END}: inherit;
...
#tabbrowser-arrowscrollbox [movetarget],
[animate-shifting] {
	--width-rounding-diff: 0px;
...
#tabbrowser-tabs[${MOVINGTAB_GROUP}] [pinned] .tab-background[multiselected] {
	outline: var(--tab-border, var(--tab-outline));
	outline-color: var(--focus-outline-color);
	outline-offset: calc(${outlineOffsetSnapping("var(--outline-width)")} * -1);
}

#tabbrowser-tabs :is([last-inflow-node], tab-group:has(> [last-inflow-node])) ~ [closing] {
	margin-inline-start: calc(-.1px - var(--forced-overflow-adjustment, 0px)) !important;
}
...
${"#tabbrowser-tabs".repeat(3)} > #pinned-drop-indicator {
	...
	z-index: calc(var(--tabs-moving-max-z-index) + 1);
	width: calc(var(--tab-icon-size) + (var(--tab-padding-inline) + var(--tab-pinned-inline-padding)) * 2);
	min-width: 0;
	height: var(--tab-min-height);
	padding: 0;
	margin: var(--tab-margin-block) var(--tab-margin-overflow-clip);
	border-radius: var(--tab-border-radius);
	outline-offset: calc(var(--snapped-border-width) * -1);
...
${prefs.tabsUnderControlButtons ? /*css*/`
				...
				clip-path:
					${Shape.create([
						...
						/*js*/`var(--top-placeholder-clip,
							var(--pre-tabs-clip,
								${Shape.hvTo(
									CSSRound(/*js*/`${START_PC} + (var(--pre-tabs-items-width) + var(--tabstrip-padding)) * ${DIR}`, "var(--snapped-border-width)"),
							...
							var(--post-tabs-clip,
								${[
									Shape.hTo(CSSRound(`${END_PC} - var(--post-tabs-items-width) * ${DIR}`, "var(--snapped-border-width)")),
									Shape.vhTo(
	...
	${prefs.floatingBackdropOpacity && !prefs.floatingBackdropClip ? /*css*/`
		...
		${cssImgTheme && prefs.tabsAtBottom < 0 ? /*css*/`
			:root[theme-image-in-toolbox] ${_}::before {
	...
	${_="#tabs-placeholder-pre-tabs"} {
		...
		inset-inline-start: 0;
		width: calc(var(--pre-tabs-items-width) - var(--nova-block-border-end, 0px));
		padding-inline-end: var(--tabstrip-padding);
	...
	${_="#tabs-placeholder-post-tabs"} {
		...
		width:
			calc(
				var(--post-tabs-items-width)
				- var(--tabs-scrollbar-width)
				- var(--scrollbar-clip-reserved)
				- var(--nova-block-border-end, 0px)
			);
		border-top-width: var(--section-border-width);
	...
	${!prefs.floatingBackdropClip ? `
		${prefs.tabsAtBottom < 0 ? `
			${BACKGROUND_ON_BODY && !isYAlign ? `
				:root:not([inFullscreen]) :where(${topPlaceholdersWithSidebarAbove}),
			` : ``}
			:root[aiwindow-immersive-view] .tabs-placeholder[position=top]
		` : `
			${
				prefs.tabsAtBottom
					? `:is(
						${prefs.tabsAtBottom > 1 ? `
							:root:not([inFullscreen]) #TabsToolbar:has(~ #PersonalToolbar:not([collapsed], [collapsed=""])),
						` : ``}
			...
			${
				prefs.tabsAtBottom
					? `:is(
						...
						` : ``}
						${!nova && BACKGROUND_ON_BODY || appVersion > 156 ? `
						...
						${!nova ? `
							#TabsToolbar:has(
								~ #notifications-toolbar > .notificationbox-stack:not(:empty)
							)
						` : ``}
					)`
					: `:is(
						${nova ? `
							:root[ai-window]:not(${aiHideNavbar}),
							:root${aiHideNavbar} #navigator-toolbox:is(
								:root:not([inFullscreen]) :has(> #PersonalToolbar:not([collapsed])),
								:has(> #notifications-toolbar > .notificationbox-stack:not(:empty))
							),
						` : `
							:root[ai-window],
						`}
	...
	@media ${twoOrLessRows} {
		...
		${prefs.tabCornerRadius > -1 ? /*css*/`
			${context} ${adjacentNewTab} .toolbarbutton-icon {
				border-radius: min(var(--toolbarbutton-padding-inner, var(--toolbarbutton-inner-padding)), var(--tab-border-radius));
			}
		` : ``}
...
${prefs.tabsAtBottom && !taskBarTab ? /*css*/`
	...
	${_ = "#TabsToolbar"} {
		...
		&${_} tab:not([selected]) {/*raise the specificity*/
			.tab-icon-overlay {
				background-image:
					/*ESR140*/
					/*image() -> linear-gradient()*/
					linear-gradient(var(--audio-overlay-extra-background)),
					linear-gradient(${nativeTheme155up ? "transparent" : "var(--toolbar-background-color, var(--toolbar-bgcolor))"}),
					linear-gradient(var(--toolbox-background-color));
			}

			.tab-note-icon-overlay {
				background-image:
					image(${nativeTheme155up ? "transparent" : "var(--toolbar-background-color, var(--toolbar-bgcolor))"}),
	...
	#nav-bar {
		...
		:root:not([ai-window]) & {
			${prefs.tabsAtBottom < 0 && prefs.toolbarColorOpacity ? /*css*/`
				&:has(~ #notifications-toolbar > .notificationbox-stack:not(:empty)),
				:root:not([inFullscreen]) &:has(~ #PersonalToolbar:not([collapsed]))
			` : `
				&
			`} {
				...
				${nova && prefs.tabsAtBottom < 0 ? /*css*/`
					...
					& ~ #PersonalToolbar[collapsed] {
						& ~ #notifications-toolbar notification-message:first-child {
							border-top: 0;
						}
					}
				` : ``}
	...
	${prefs.tabsAtBottom < 0 ? /*css*/`
		${_} {
			...
			` : /*css*/`
				.urlbar[popover-open] {
					.urlbar-background {
						border-radius: var(--urlbar-background-border-top-radius-open);
						border-start-start-radius: 0;
						border-start-end-radius: 0;
						inset-block:
							calc(-1 * var(--urlbar-joint-background-overlap))
							calc(-1 * (var(--urlbar-background-overhang-open) + var(--urlbar-input-growth-open-block)));
						clip-path:
							inset(
								calc(var(--urlbar-joint-background-overlap) - var(--urlbar-input-growth-open-block))
								${__="calc(-1 * var(--urlbar-joint-background-shadow-margin))"}
								${__}
							);
					}
						...
						.urlbarView-background {
							...
							clip-path:
								inset(
									${__}
									${__}
									var(--urlbar-joint-background-overlap)
								);
						}
					}
				}
		...
		${nova ? /*css*/`
			...
			${
				//workaround for firefox bug that the placeholders can't show bluriness
				//it has a drawback that the background covers the box-shadow of browser content
				!prefs.floatingBackdropClip &&
				prefs.floatingBackdropBlurriness &&
				prefs.floatingBackdropOpacity < 100
					? /*css*/`
						:root[ai-window]:not([aiwindow-immersive-view]) ${_} {
							${toolbarBackgroundStyle}
						}
					` : ``
			}
		...
		${(
			...
			:root${BACKGROUND_ON_BODY ? `[${THEME_IMAGE_IN_TOOLBOX}]:not([ai-window])` : ""}
			{
				${_} {
					${layeredBackgroundSytle}
					transition: background-color var(--inactive-window-transition);
					...
					${nativeTheme155up ? `
						--chrome-block-${isYAlign && bgImgAllRepeat ? "background" : "toolbar"}-color:
							var(--toolbox-background-color-current);
					` : /*css*/`
						&, .tabs-placeholder::before {
							--chrome-block-toolbar-color: var(--toolbar-background-color, var(--toolbar-bgcolor));
						}
		...
		:root:not([inDOMFullscreen]) :is(#browser, #customization-container) {
			...
			margin-bottom:
				calc(
					var(--tabstrip-min-height) * var(--tab-rows)
					+ var(--margin-bottom)
				);

			${nova ? /*css*/`
				#sidebar-box, #sidebar-box #sidebar {
					border-end-start-radius: var(--sidebar-box-start-start-radius);
					border-end-end-radius: var(--sidebar-box-start-end-radius);
				}
			` : ``}
		...
		@media -moz-pref("browser.fullscreen.autohide") {
			:root[inFullscreen]:not([animating-tabs-slot-size]) {
				...
				&:not(:has(
					...
					${TABS_RELATED_PANELS.split(",").map(s =>
						`> body > #mainPopupSet > ${s.trim()}[panelopen]`
					).join(",\n")},
					> body > #mainPopupSet > menupopup[anchor-to-tabsbar]
				)) {
					--tab-bar-hiding-transition: 0s;

					&[fullscreen-fully-entered] {
						--tab-bar-hiding-transition: 0s .4s;
					}
					...
					#TabsToolbar {
						...
						--height: 1px;
						filter: opacity(0);
						transition: var(--tab-bar-hiding-transition) !important;
						transition-property: clip-path, filter !important;

						[breakout] {
							visibility: hidden;
							transition: visibility var(--tab-bar-hiding-transition);
						}
					}
		...
		:root:not([inDOMFullscreen]) #tabbrowser-tabpanels > :not(.split-view-panel) .browserContainer {
			@media -moz-pref("sidebar.revamp")
				${roundConentCanDisable ? `and -moz-pref("sidebar.revamp.round-content-area")` : ``}
			{
				${HAS_AUTO_UI_DENSITY ? `:root:not([inFullscreen])` : ``}
					#tabbrowser-tabbox[sidebar-shown] &
				{
					border-end-${sidebarAtStart ? "start" : "end"}-radius:
						var(--border-radius-medium);
				}
			}
		}

		:root:not([inDOMFullscreen]) #tabbrowser-tabbox[sidebar-shown] .dialogOverlay[topmost] {
			.browserSidebarContainer:not(
				.responsive-mode, [devtools-host-type=bottom], [devtools-host-type=${sidebarAtStart ? START : END}]
			)
				${roundConentCanDisable ? `@media -moz-pref("sidebar.revamp.round-content-area")` : `&`}
			{
				border-end-${sidebarAtStart ? "start" : "end"}-radius:
					${nova
						? /*js*/`calc(var(--border-radius-medium) - var(--snapped-border-width))`
						: `var(--border-radius-medium)`};
			}
		}
		...
		#tab-preview-panel {
			${nova ? `
				--panel-padding: var(--space-small) var(--space-small) 0;
			` : ``}
			...
			.tab-preview-content-interactive {
				...
				&:not(:empty) {
					padding-block: 0 var(--space-small);
				}
			}
		}
	` : ``}
` : !prefs.hamburgerMenuOnTabBar ? /*css*/`
	:root[ai-window] {
		#nav-bar {
			padding-inline-end: 0;

			#smartwindow-ask-button {
				padding-inline: var(--smartwindow-ask-btn-inline-start-padding);
			}
		}
	}
` : ``}

${prefs.toolbarColorOpacity < 100 ? /*css*/`
	:root:not([ai-window]) body {
		--tabs-navbar-separator-color:
			color-mix(
				in srgb,
				var(--original-tabs-navbar-separator-color)
					${prefs.toolbarColorOpacity}%,
				transparent
			);
	...
	${win7 || win8 ? /*css*/`
		#tabbrowser-tabpanels {
			--toolbar-bgcolor: var(--original-toolbar-background-color);
		}
	` : ``}
` : ``}

${prefs.urlBarColorOpacity < 100 ? /*css*/`
	:root:not([ai-window]) :is(
		...
		background-color:
			color-mix(
				...
				var(--urlbar-background-color,
					...
					${prefs.urlBarColorOpacity}%,
				transparent
			);
	}
` : ``}

${cssImgTheme && prefs.themeGradientOpacity < 100 ? /*css*/`
	:root:not([ai-window], [multitabrows-changing-gradient]) {
		${(() => {
			let opacity = prefs.themeGradientOpacity;
			root.setAttribute("multitabrows-changing-gradient", "");
			let r = ["--lwt-additional-images", "--toolbox-background-gradient"]
				.map(replaceGradientColors).join("\n");
			root.removeAttribute("multitabrows-changing-gradient");
			return r;

			function replaceGradientColors(property) {
				if (!opacity)
					return `${property}: none !important;`;

				let gradient = rootCS.getPropertyValue(property);
				if (["none", "", "image(transparent)"].includes(gradient))
					return "";
				try {
					...
					let value = `${parsed.type}(${
						[
							...
							...parsed.stops.map(s =>
								[
									`color-mix(in srgb, ${s.color} ${opacity}%, transparent)`,
									s.stop,
								].filter(v => v).join(" ")
							),
						].filter(v => v).join(", ")
					...
					window.console.error({property, gradient}, e);
					return "";
				}
			}
			...
			function parseGradient(gradient) {
				...
				let prefix = null, stops = [];

				if (
					parts[0] &&
					/^[\d.]+(deg|rad|turn)$|^(to|from|at)\s|^(circle|ellipse)$|^var\(/i.test(parts[0])
				) {
					prefix = parts[0];
					stops = parts.slice(1);
				} else
				...
				const parsedStops = stops.map(part => {
					let color = part;
					let stop = null;
					const stopMatch = part.match(
						/\s*(.*)\s+([\d.]+(?:%|px|v(?:w|h|min|max))|(?:var|calc|min|max|clamp|round)\([^)]+\))$/i
					);
					if (stopMatch)
						[, color, stop] = stopMatch;
					return { color, stop };
				});

				return { type, prefix, stops: parsedStops };
			}
		})()}
	}
` : ``}
...
${prefs.nativeWindowStyle ? /*css*/`
	...
	${context} {
		...
		${BACKGROUND_ON_BODY && micaTheme ? /*css*/`
...
${debug && debug < 3 && debug > 0 ? /*css*/`
	...
	tab-split-view-wrapper::after,
	:not(tab-split-view-wrapper) > tab .tab-background {
		opacity: .7;
...
${debug < 0 ? /*css*/`
	:root {
		...
		--lwt-additional-images: var(--toolbox-background-gradient) !important;
		${debug < -1 ? `
			--toolbox-background-gradient: linear-gradient(
				90deg,
				green 5%, red 0,
...
{
	new ResizeObserver(updateNavToolboxSizeInfo).observe(gNavToolbox);
			...
			if (maxHeight) {
				...
				let bgPos = cs.backgroundPosition.split(", ").map((s, i) => {
					...
					if (s[1] != "0%") {
						...
						s[1] = `var(--nav-toolbox-margin-top)`;
...
/** arrowScrollbox **/
{
	...
	for (let handler of ["overflow", "underflow", "touchstart", "touchend"])
		arrowScrollbox["on_" + handler] = emptyFunc;

	arrowScrollbox.on_touchmove = function(event) {
		if (tabContainer.isMovingTab || event.target.closest("[touchdownstartsdrag]"))
	...
	arrowScrollbox.on_scrollend = function(e) {
		if (e.target != scrollbox || this._touchStart >= 0)
			return;
...
/** workaround with full screen **/
{
	...
	FullScreen.toggle = function() {
		...
		if (on) {
			waitForTransition(gNavToolbox, "margin-top").then(() => {
				root.setAttribute("fullscreen-fully-entered", "");
				updateNavToolboxSizeInfo();
			});
		} else {
			updateNavToolboxSizeInfo();
			root.removeAttribute("fullscreen-fully-entered");
		}
...
if (splitViewProto) {
	define(splitViewProto, {
		...
		index: function() {
			return this.visibleTabs[0]?.index;
		},
	...
	const elementIndexOpd = {
		set: function(v) {
			...
			group.overflowContainer?.setAttribute(
				"elementIndex",
				v + group.visibleTabLikes.length + .5,
			);
		},
	...
	define(groupProto, {
		...
		collapsed: {
			...
			set: function(v) {
				...
				let shouldLockTabSize = this[CLICKED_BY_MOUSE] || tabContainer.isMovingTab;
				this.togglingAnimation = animateLayout(() => {
					if (!v)
						this.stacked = false;
					else if (shouldLockTabSize) {
					...
					excludeNodes:
						dragging
							? [
								...
								//reduced the number of animations to minimize performance impact
								...this.visibleTabLikes.slice(5),
							]
							: !v
								? this.overflowContainer
								: null,
					includeNodes: !v ? this.nonHiddenTabLikes.filter(t => !t.visible) : null,
				}).finally(() => {
					delete this.togglingAnimation;
					if (shouldLockTabSize)
						delete tabContainer._keepTabSizeLocked;
					this.setAttribute("toggling", v ? "collapsed" : "expanded")
					requestAnimationFrame(() => this.removeAttribute("toggling"));
				});
			},
		},
	...
	assign(groupProto, {
		...
		addTabs: function(tabs) {
			...
			if (isCalledBy("on_drop"))
				...
				animateLayout(async () => {
					let {animateLayout} = tabs[0]?.container || {};
					if (
						animateLayout &&
						tabs[0][DOCUMENT_GLOBAL] != window
					)
						animateLayout(run);
					else
						run();
		...
		remove: function() {
			...
			}).finally(() => delete this.removingAnimation);
		},
...
let GET_DRAG_TARGET;
{
	...
	dragDropProto.startTabDrag = function(e, tab, opt, ...args) {
		...
		//the scrollbox may scroll a bit during touch move
		...
		} else if (tab.group?.hasAttribute("toggling"))
			return;
	...
	dragDropProto[`${HANDLE}_dragover`] = function(e) {
		...
		const allowIndToPin = HAS_DRAG_TO_PIN && allowToPin;
	...
	dragDropProto._animateTabMove = async function(e) {
		...
		const dragToGroupTabs = this._tabGroupDragToCreate && gBrowser.tabGroupsEnabled;
		...
		if (lastX == null) {
			...
			let moveOverThreshold = gBrowser.tabGroupsEnabled
			...
			assign(_dragData, {
				...
				moveOverThreshold,
				groupThreshold,
				pinInteractionCue: getPref("browser.tabs.dragDrop.pinInteractionCue.delayMS", 350),
			});
		...
		if (!tabsBar.hasAttribute("movingtab")) {
			...
			if (isSplitViewWrapper(draggedNode))
				InspectorUtils.addPseudoClassLock(
					$("tab:is(:active, [touchdownstartsdrag])", draggedNode),
					":hover",
				);
		...
		if (shouldMove) {
			...
			await animateLayout(async () => {
				...
				if (
					rowChange <= 0 &&
					!recursion &&
					_dragData.layoutLockedRow != cursorRow &&
					(!draggingTab || _dragData.stacking || movingNodes.length == 1)
				) {
					tabContainer._lockTabSizing(movingNodes.find(n => !n.stacking), {lockPreviousRows: rowChange - 1});
				...
				if (!recursion) {
					if (
						...
						for (let [node, rect] of nodeRects)
							if (rowChange > 0 || rect.row >= cursorRow)
								tabContainer._setLockedSize(node);
				...
				if (animatingLayout) {
					...
					if (collapsingTabs[0]) {
						...
						for (let t of collapsingTabs) {
							...
							let postActive = t.index > activeTab.index;
	...
	dragDropProto._clearGrouping = function(_dragData) {
		if (!this._tabGroupDragToCreate || !gBrowser.tabGroupsEnabled)
			return;
	...
	if (_recoverFromStaleDrag)
		dragDropProto._recoverFromStaleDrag = function() {
			if (tabContainer.isFinishingTabMove)
				return;
			let shouldRecover = tabContainer.matches("[dragging], [movingtab]");
			_recoverFromStaleDrag.apply(this, arguments);
			if (shouldRecover)
				this._postDraggingCleanup();
		};
	...
	dragDropProto[`${HANDLE}_drop`] = function(event) {
		...
		let run = async () => {
			...
			if (
				...
				// weird bug: when copying a grouped tab to a collased active group in another window,
				// the new copy of tab will disapear after unlocking is called
				!(sameWindow && tabContainer.suggestToLockSlot)
			)
		...
		if (draggedTab && !sameWindow)
			animateLayout(() => {
				let {container} = draggedTab;
				if (container.animateLayout)
					container.animateLayout(run);
				else {
					run();
					container.tabDragAndDrop?._resetTabsAfterDrop?.();
				}
			});
			...
			if (_dragData.shouldDropIntoCollapsedTabGroup)
				...
				if (_dragData.shouldCreateGroupOnDrop)
					...
					animateLayout(() => {
						...
						tabContainer.style.pointerEvents = ""
					);
			}
		} else if (useIndicatorToMove)
			...
			try {
				...
				if (newPinState != null && isCalledBy("on_drop")) {
					...
					if (rectsBeforeDrop && movingNodes[0].group) {
						...
						let nodes = [...rectsBeforeDrop.keys()];

						await animateLayout(() => {
							delete tabContainer._keepTabSizeLocked;
							tabContainer._unlockTabSizing({instant: true, unlockSlot: false});
							movingNodes[0].group.refreshState();
						}, {
							oldNodes: nodes,
							nodes,
...
/** tabContainer **/
{
	...
	tabContainer._positionPinnedTabs = function(numPinned, stopRecursion) {
		...
		if (floatPinnedTabs) {
			...
			if (!stopRecursion && !overflowing && scrollbox.scrollTopMax) {
	...
	*/
	...
	tabContainer.refreshAllGroups = function() {
		if (HAS_TAB_GROUP)
			for (let g of gBrowser.tabGroups)
				g.refreshState();
	};
	...
	let placeholderStyle = assign(document.body.appendChild(document.createElement("style")), {id: "multiTabRows-placeholderStyle"});

	tabContainer.updateLayout = function(recursionTimes = 0) {
		...
		const wasPositioningPinnedTabs = this.positioningPinnedTabs;
		...
		if (
			...
			css = css.join("");
			console?.debug(css);
			if (placeholderStyle.textContent != css)
				placeholderStyle.textContent = css;
		...
		if (recursionTimes < 10) {
			if (
				//overflow may be fired when moving tab then locking the slot
				lastLayoutData.overflowing && !prvOverflowing && !this.isMovingTab ||
				pointDelta(prvSlotWidth, lastLayoutData.slotWidth)
			) {
				this._unlockTabSizing({instant: true, skipUpdateLayout: true});
				console?.warn(
					"recurse updateLayout",
					{
						prvSlotWidth, prvOverflowing,
						slotWidth: lastLayoutData.slotWidth,
						overflowing: lastLayoutData.overflowing,
					},
				);
				this.updateLayout(++recursionTimes);
			} else if (
				//in some extreme cases it overflows again after updating state
				wasPositioningPinnedTabs && prefs.positionPinnedTabs &&
				lastLayoutData.overflowing && !this.positioningPinnedTabs
			)
				this.updateLayout(++recursionTimes);
		} else
			window.console.error("Too many recursion of updateLayout!");
	...
	tabContainer._updateState = function() {
					...
					!!(
						...
						(
							...
							(
								...
								$(`
									#TabsToolbar-customization-target >
										toolbarspring:nth-last-child(1 of :not([hidden], [collapsed]))
								`)
							)
						)
					),
		...
		root.setAttribute("tab-scroll-rows", scrollRowCount);
	...
	tabContainer.addEventListener("TabGroupUpdate", function(e) {
		...
		if (gBrowser.tabGroupMenu.panel.hasAttribute("panelopen"))
			e.target.labelElement.scrollIntoView();
	});
	...
	tabContainer._lockTabSizing = function(actionNode, {lockPreviousRows, stacking} = {}) {
		...
		if (nodes[0]) {
			...
			if (lockPreviousRows) {
				...
				for (let n of nodes) {
					...
					if (deltaY >= 0)
						break;
					if (deltaY / tabHeight >= lockPreviousRows)
						sizes.set(n, r.width);
				}
				...
				if (
					...
					if (hasAdjacentNewTab && !(prefs.tabsUnderControlButtons < 2 && this.overflowing))
				...
				for (let n of nodes) {
					...
					if (
						...
						(
							...
							(prefs.dragStackPreceding || n.index > selectedNode.index)
						)
					)
						continue;
					...
					if (deltaY > 0) {
						if (unifiedSplitViewWidth == Infinity)
							unifiedSplitViewWidth = Math.max(unifiedWidth, splitViewMinWidth);
						else if (unifiedWidth == Infinity)
							unifiedWidth = tabMinWidth;
					}
	...
	tabContainer._unlockTabSizing = async function({
		...
		if (
			...
			(
				...
				$$(`${TABS_RELATED_PANELS}, #tabContextMenu`)
					.some(p =>
						["open", "hiding"].includes(p.state) &&
						(p.id != "tab-preview-panel" || p.matches(":hover"))
					)
			)
		)
			return;
		...
		await animateLayout(async () => {
			...
			shouldUpdateLayout: !skipUpdateLayout,
		});
	...
	tabContainer[ON_UI_DENSITY_CHANGED] = function() {
		...
		style(
			...
			Object.fromEntries(
				Object.entries({
					tabContentHeight: "--tab-min-height",
					tabVerticalMargin: __TAB_MARGIN_BLOCK,
					tabHorizontalPadding: __TAB_PADDING_INLINE,
				}).map(([prop, name]) => [
					name,
					suffix || Services.prefs.prefHasUserValue(prefBranchStr + prop)
						? prefs[prop + suffix] + "px"
						: "",
				])
			),
		);
		...
		style(this, {
			[propToStore.splitViewMinWidth]: "var(--splitview-min-width) !important",
...
/** gBrowser **/
{
	...
	if (HAS_TAB_GROUP && !("tabGroupsEnabled" in gBrowser))
	...
	let {
		...
		replaceTabWithWindow, replaceTabsWithWindow, replaceGroupWithWindow,
	...
	gBrowser.removeTab = function(tab) {
		...
		let closing;
		if (!visibleTabs[1] && visibleTabs[0] == tab && !gBrowser.tabGroups?.[0])
			closingTheOnlyTab = true;

		if (animatingLayout || !visibleTabs.includes(tab) || gReduceMotion) {
			// On Firefox 155+, opening Firefox View for the first time triggers
			// the removal of the view tab. It causes the tab to blur and scrolls to another tab.
			// Not sure why the scrolling doesn't occur on native Firefox when no breakpoint is set.
			if (tab && FirefoxViewHandler.tab == tab)
				removingFirefoxViewTab = true;

			removeTab.apply(this, arguments);
		} else {
			let {splitview} = tab;
			let removeMulti = isCalledBy("removeMultiSelectedTabs");
			let hasPrompted;
			...
			animateLayout(self => {
				const {nodes, rects, newRects, tabsRect, scrollTop, action} = animatingLayout || {};
				removeTab.apply(this, arguments);
				...
				if (!closing || !tab.isConnected) {
					...
					else if (closingTheOnlyTab) {
						let newTab = gBrowser.visibleTabs[0];
						if (newTab) {
							rects.set(newTab, rects.get(tab));
							nodes.push(newTab);
							animatingLayout.includeNodes.push(newTab);
						}
					}
				} else {
					...
					if (!splitview && !removeMulti) {
						let newTabsRect = getRect(tabContainer, {box: "content"});
						let newScrollTop = scrollbox.scrollTop;
						let oldR = rects.get(tab);
						let newR = getRect(tab);
						if (pointDelta(oldR.y - tabsRect.y + scrollTop, newR.y - newTabsRect.y + newScrollTop)) {
							let nxtNode = nodes.slice(nodes.indexOf(tab) + 1).find(n => !n.closing && n.isConnected);
							newR = nxtNode && nxtNode != tabContainer.newTabButton
								? getRect(nxtNode) : oldR.clone();
							newR.start = prefs.justifyCenter > 1
								? newR.start + newTabsRect.start - tabsRect.start
								: getRect(slot, {box: "content"}).start;
							newR.y += newTabsRect.y - newScrollTop - tabsRect.y + scrollTop;
							newR.width = 0;
							tab._animationTargetingRect = newR;
						} else
							newRects.set(tab, newR);
					}
				}
		...
		removingFirefoxViewTab = closingTheOnlyTab = false;
	};
	...
	//for 156+, the method becomes private and not modifying it seems have no side effects so far, not so sure
	...
	gBrowser.addTabGroup &&= function() {
		let g;
		let run = () => g = addTabGroup.apply(this, arguments);
		if (isCalledBy("on_drop"))
			return run();
		animateLayout(run);
		return g;
	...
	gBrowser.replaceTabWithWindow = function(tab) {
		let w;
		animateLayout(() => {
			w = replaceTabWithWindow.apply(this, arguments);
			if (w && isTabLike(tab)) {
				tab.setAttribute("fadein", true);
				tab.setAttribute("closing", "");
			}
	...
	gBrowser.adoptTabGroup &&= function(group) {
		...
		animateLayout(() => {
			...
			let {animateLayout} = group?.labelElement.container || {};
			if (
				animateLayout &&
				group[DOCUMENT_GLOBAL] != window
			)
				animateLayout(run);
			else
				run();
	...
	function inject() {
		for (let panel of [gTabsPanel.groupsPanel, gTabsPanel.showAllGroupsPanel]) {
			...
			panel.handleEvent = function(e) {
				...
				if (
					e.type == "command" &&
					e.target?.dataset.command == "allTabsGroupView_restoreGroup"
				)
					animateLayout(run);
				else
					run();
...
$("#saved-tab-group-context-menu_openInThisWindow")
	?.addEventListener("command", () => animateLayout(async () => await rAF()), true);
...
/** TabBarVisibility **/
{
	let {update} = TabBarVisibility;
	TabBarVisibility.update = function() {
...
if (HAS_AUTO_UI_DENSITY) {
	define(gUIDensity, {
		AUTO_COMPACT_REFERENCE_TABSTRIP_HEIGHT: {
			get: () => prefs.tabContentHeightCompact + prefs.tabVerticalMarginCompact * 2,
			set: () => window.console.error("AUTO_COMPACT_REFERENCE_TABSTRIP_HEIGHT is read-only."),
		},
	});
	gUIDensity.update();
}
...
function identifyTitleBar() {
	tabsBar.classList.toggle("browser-titlebar", !prefs.tabsAtBottom);
	$("#nav-bar").classList.toggle("browser-titlebar", prefs.tabsAtBottom);
}

function updateNavToolboxSizeInfo() {
	let padding = getRect(gNavToolbox, {box: "padding"});
	let border = getRect(gNavToolbox, {box: "border"});
	let margin = fullScreen ? border : getRect(gNavToolbox, {box: "margin"});
	style(root, {
		"--browser-content-top": getRect($("#browser"), {box: "content"}).y + "px",
		"--nav-toolbox-border-box-height": border.height + "px",
		"--nav-toolbox-border-top": padding.y - border.y + "px",
		"--nav-toolbox-border-bottom": border.bottom - padding.bottom + "px",
		"--nav-toolbox-margin-top": border.y - margin.y + "px",
		"--nav-toolbox-margin-bottom": margin.bottom - border.bottom + "px",
		"--nav-toolbox-margin-inline": border.x - margin.x + "px",
	});
}
	...
	if (
		...
		nodes = $$(`
			:is(
				...
				${includeClosing ? `
					tab-group:is(
						[toggling=collapse]:not([stacked]),
						[stacked=from-expanded]
					),
				` : ``}
				tab-group:not([collapsed])
			) >
			...
			` : ``}
			.tab-group-label-container,
			tab-group[collapsed][hasactivetab]:not([stacked]) >
				:is(
					tab[selected],
					tab-split-view-wrapper[hasactivetab]
				)
...
function isAutoUIDensity() {
	return nova && HAS_AUTO_UI_DENSITY &&
...
function updateThemeStatus(themeData = getThemeData()) {
	...
	nova = novaMQ.matches || HAS_NOVA && !Services.prefs.prefHasDefaultValue("browser.nova.enabled");
	defaultTheme =
		!tempTheme &&
	...
	nativeTheme155up = appVersion > 154 && nativeThemeMQ.matches;
	useAccentColor =
		defaultAutoTheme &&
		accentColorInTitlebarMQ.matches &&
		(
			micaEnabled ||
			nativeThemeMQ.matches ||
			getPref("browser.theme.windows.accent-color-in-tabs.enabled")
		);
	mica = micaEnabled && defaultAutoTheme && !useAccentColor;
	micaTheme =
	...
	bgImgTheme = !!(
		theme?.headerImage ||
		theme?.headerURL ||
		theme?.additionalBackgrounds?.some(v => typeof v == "string")
	);
	...
	cssImgTheme =
		(
			nova && defaultTheme && !nativeTheme155up &&
			!(appVersion > 157 && (useAccentColor || mica))
		) ||
		(
			theme?.backgroundsAlignment?.split(",").some(a =>
				["center", "bottom"].includes(a.split(" ")[1] || a)
			) &&
			theme?.additionalBackgrounds?.[0] &&
			theme?.additionalBackgrounds.every(v =>
				Object.keys(v ?? {})[0]?.endsWith("-gradient")
			)
		);
}
...
async function animateLayout(
	...
	if (animate) {
		...
		if (includeNodes)
			nodes = [...new Set([
				...nodes,
				...(includeNodes = [includeNodes].flat()),
			])];
		else
			includeNodes = [];
		...
		animatingLayout = {
			...
			includeNodes, excludeNodes, oldNodes,
	...
	try {
		...
		if (animate) {
			({
				cancel, shouldUpdateLayout, includeNodes, excludeNodes,
			} = animatingLayout);
			animatingLayout = null;
		}
	}
	...
	return new Promise(async rs => {
		...
		if (deltaNodes) {
			...
			if (deltaNodes.every(n => n.index > -1))
				deltaNodes.sort((a, b) => a.index - b.index);
		} else
			deltaNodes = newNodes.filter(n =>
				!includeNodes?.includes(n) &&
				!oldNodes.includes(n) &&
				!(isSplitViewWrapper(n) && n.tabs.some(t => oldNodes.includes(t)))
			);
		...
		if (deltaNodes[0]) {
			...
			for (let i in deltaNodes) {
				...
				if (isTabGroupOverflowContainer(n)) {
					...
					!oldNodes[1] && isTabLike(oldNodes[0]) &&
					!newNodes[1] && isTabLike(newNodes[0])
				)
					rects.set(n, rects.get(oldNodes[0]));
				else {
					let r;
					...
					let prv = i || isTabGroupLabelContainer(n)
						...
						: nxt
							? oldNodes[oldNodes.lastIndexOf(nxt) - 1]
							: oldNodes.at(-1);
					let {group} = n;
					if (group?.collapsed) {
						let label = group.labelContainerElement;
						if (
							getGroup(prv) != group ||
							(
								prv != label &&
								(() => {
									let tabLikes = group.nonHiddenTabLikes;
									return tabLikes.indexOf(prv) > tabLikes.indexOf(n);
								})()
							)
						)
							prv = label;
					...
					if (
						prvNewR?.visible &&
						(
							!pointDelta(prvNewR.y, newR.y) ||
							["expand", "expanded"].includes(group?.getAttribute("toggling"))
						)
					) {
						...
						r.start =
							n == newNodes[0] ?
								prefs.justifyCenter > 0 ?
									newSlotRect.centerX
									+ (
										(
											lastLayoutData.preTabsItemsSize
											+ lastLayoutData.tabsStartSeparator
											- lastLayoutData.postTabsItemsSize
										) / 2 * DIR ||
										0
									)
								:
									...
									+ (
										(
											(lastLayoutData.preTabsItemsSize + lastLayoutData.tabsStartSeparator)
											* DIR
										) ||
										0
									)
		...
		let run = () => Promise.all(nodes.map(n => {
			...
			if (targetingR) {
				n.style.transform = `translate(
					${targetingR.start - nR.start}px,
					${targetingR.y - nR.y}px
				)`;
				nR = targetingR;
			}
		...
		nodes = nodes.filter(n => {
			...
			window.console.error("node lack of rect", {nR, oR});
		});
...
async function animateShifting(n, oR, nR) {
	...
	if (n[ANIMATE_REQUEST]?.every((v, i) => abs(v - to[i]) < 1)) {
		let prvAni = await waitForAnimate(n);
		if (prvAni.some(a => a)) {
	...
	if (n[ANIMATE_REQUEST] == to) {
		...
		let anis = await waitForAnimate(n);

		if (n[ANIMATE_REQUEST] == to || anis.some(a => a)) {
			n.removeAttribute("animate-shifting");
			style(n, {"--x": "", "--y": "", "--w": "", "--l": ""});
...
async function waitForTransition(node, property, pseudo) {
	...
	// eslint-disable-next-line no-unused-vars
		...
		let nextAni = node[APPLYING_ANIMATION];
		if (nextAni) {
 ...
 * @param {boolean} [fallbackWhenNoDefault]
 * @returns {(number|boolean|string)}
 */
function getPref(name, fallback, fallbackWhenNoDefault) {
	let {prefs} = Services;
	let type = prefs.getPrefType(name);
	if (!type || fallbackWhenNoDefault && !prefs.prefHasDefaultValue(name))
		return fallback;
	return prefs[
		`get${{
...
async function getScriptInfo({
	...
	let get = field => script.match(new RegExp(`^//\\s*@${field}\\s+(.+?)\\s*$`, "mi"))[1];
	let version = get("version")
		.replace(/((\d+\.\d+\.\d+)\.\d+)/, skipMinorVer ? "$2" : "$1")
		.replace(/(\.0)+$/, "");
	let compatibility = get("compatibility")
		.replace(/firefox\s*/i, "")
		.split(/\s*,\s*/).map(v => v.split(/\s*-\s*/));
	let compatible =
		!!prefs.checkUpdate && appVersion > compatibility.at(-1).at(-1) ||
