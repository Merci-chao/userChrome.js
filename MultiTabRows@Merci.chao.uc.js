...
// @author         Merci chao
// @version        4.4
// @compatibility  Firefox 115, 147-149
...
/* global
   ...
   gNotificationBox, gTabsPanel, AIWindow
*/
...
function loadPrefs(defaultPrefs = createDefaultPrefs()) {
	...
	let moveOverThreshold = getPref("browser.tabs.dragDrop.moveOverThresholdPercent", 0);
	let singleRow = prefs.maxTabRows < 2;
	let nativeDragToGroup = moveOverThreshold > 50 && getPref("browser.tabs.groups.enabled", false);

	let supportsHas = getPref("layout.css.has-selector.enabled", true);
	...
	lock("tabsbarItemsAlign", singleRow || prefs.tabsUnderControlButtons > 1 || prefs.autoCollapse);
	lock(
		...
		(
			...
			(mica || prefs.nativeWindowStyle) && !getPref("browser.smartwindow.enabled")
		),
	);
	...
	lock("smartWindowButtonOnNavBar", prefs.tabsAtBottom || !getPref("browser.smartwindow.enabled"), prefs.tabsAtBottom);
	...
	lock("dragStackPreceding", !getPref("browser.tabs.dragDrop.multiselectStacking", TAB_STACKING_SUPPORT) || maxMove < 2);
	lock(
		["spaceAfterTabs", "spaceAfterTabsOnMaximizedWindow", "spaceBeforeTabs", "spaceBeforeTabsOnMaximizedWindow"],
		!getPref("browser.tabs.inTitlebar") || $(`#toolbar-menubar:not(${MENUBAR_AUTOHIDE})`),
		0,
	);
	lock(
	...
	lock("previewPanelNoteEditable", !getPref("browser.tabs.notes.enabled"));
	lock("positionPinnedTabs", prefs.pinnedTabsFlexWidth || prefs.autoCollapse, false);
	lock("lastRowTabsFlexibe", prefs.justifyCenter == 2);
	lock("previewPanelShiftedAlways", !prefs.previewPanelShifted);
	lock("animateTabMoveShiftKeyToPause", !prefs.animateTabMoveMaxCount);
...
async function onPrefChange(pref, type, name) {
	...
	switch (name) {
		...
		// case "dlpButtonOnNavBar":
			setStyle();
			tabContainer.updateLayout();
			updateAIWindowSwitcherPosition();
			updateNavBarOverflow()
			break;
		...
		case "browser.tabs.dragDrop.multiselectStacking":
			Object.getPrototypeOf(tabContainer.tabDragAndDrop).multiselectStacking =
				getPref(name, TAB_STACKING_SUPPORT);
			break;
...
const autoHideBookmarksBar = getPref("browser.toolbars.bookmarks.visibility") == "newtab";
...
${prefs.tabsAtBottom/*dlpButtonOnNavBar*/ ? /*css*/`
	:root[contentanalysisactive] {
		#navigator-toolbox #nav-bar .content-analysis-indicator {
			display: flex;
		}
...
${prefs.showScrollSahdow ? /*css*/`
	${_}::before,
	${_}::after {
		--pos: top;
		--color: light-dark(rgba(0, 0, 0, .05), rgba(255, 255, 255, .05));
		content: "";
		/*style of overflow-*-indicator in vertical tabs mode*/
		background-image: linear-gradient(to var(--pos), transparent 0%, var(--color));
		background-repeat: no-repeat;
		height: calc(var(--tabstrip-min-height) / 2.75);
		pointer-events: none;
		position: absolute;
...
#tabbrowser-tabs[animating-slot-size] ${_} {
	overflow: hidden;
}
...
${win7 || win8 || prefs.tabsUnderControlButtons < 2 || !getPref("widget.windows.overlay-scrollbars.enabled") ? /*css*/`
	/*
...
${_}::part(scrollbutton-up)::before,
${_}::part(scrollbutton-down)::before {
	top: calc(var(--extra-scroll-button-size) * -1);
}
...
#TabsToolbar #tabbrowser-tabs tab-group {
...
${appVersion > 136 ? /*css*/`
	...
	.tab-note-icon-overlay {
		...
		inset-inline:
			calc(
				var(--tab-icon-size)
				- round(up, var(--tab-overlay-icon-size) / 2, 1px)
			)
			auto;
...
#TabsToolbar #tabbrowser-tabs tab-split-view-wrapper {
	--w: 0px;
	...
	&:is(
		...
		#tabbrowser-tabs[movingtab-group]
			:is([dragover-groupTarget], [movetarget]),
		#tabbrowser-tabs:not([moving-single-tab], [movingtab-finishing])
	...
	tab {
		...
		&:has(+ &[closing])::after {
			...
			width: calc(var(--tab-overflow-clip-margin) * 3);
		...
		&:has(+ &:not([closing])) {
			...
			box-shadow:
				calc((var(--tab-block-margin) + ${borderSnapping("var(--splitview-outline-width)")}) * ${DIR})
				0
				0
...
#tabbrowser-tabs[orient] ${_}[fadein]:is(
	...
	&:not(${__ = "[image], [crashed], [sharing], [pictureinpicture], [busy], [pendingicon]"}) {
		...
		&[tab-note]:is(${showAudioButton}) {
			.tab-content {
				--tab-icon-end-margin: round(down, var(--tab-overlay-icon-size) / 2, 1px);
			}
...
#TabsToolbar #tabbrowser-tabs > #pinned-drop-indicator {
	display: flex;
	align-items: center;
	position: absolute;
	top: var(--extra-drag-space);
...
${prefs.tabsUnderControlButtons ? /*css*/`
	...
	${(micaEnabled && accentColorInTitlebarMQ.matches || prefs.nativeWindowStyle) && defaultAutoTheme ? /*css*/`
		...
		${prefs.floatingBackdropClip && prefs.tabsUnderControlButtons > 1 ? /*css*/`
			${context="#TabsToolbar"+showPlaceHolder}:not(${tbDraggingHidePlaceHolder}) ${_="#tabbrowser-tabs"} {
				clip-path: polygon(
					...
					${x} ${y=/*js*/`calc(var(--control-box-reserved-height) - var(--tabs-top-space))`},
					${x=/*js*/`calc(${END_PC} - (var(--control-box-margin-end) + var(--control-box-radius-end)) * ${DIR})`} ${y},
					${x=/*js*/`calc(${END_PC} - var(--control-box-margin-end) * ${DIR})`} calc(var(--control-box-reserved-height) - var(--tabs-top-space) - var(--control-box-radius-end)),
					${x} 0,
					${x=END_PC} 0,
					${x} 100%,
			...
			${context}:not([tabs-scrolledtoend]) ${_}[overflow][hasadjacentnewtabbutton] {
				--new-tab-clip:
					...
					${x} ${y=/*js*/`calc(100% - var(--tabstrip-min-height))`},
	...
	${win7 || win8 ? /*css*/`
			...
			#tabbrowser-tabs[overflow]
		{
			clip-path: polygon(
				...
				${x} ${y=/*js*/`calc(var(--control-box-reserved-height) - var(--tabs-top-space))`},
				${x=/*js*/`calc(${END_PC} - (var(--control-box-margin-end) + var(--control-box-radius-end)) * ${DIR})`} ${y},
				${x=/*js*/`calc(${END_PC} - var(--control-box-margin-end) * ${DIR})`} ${y=/*js*/`calc(var(--control-box-reserved-height) - var(--tabs-top-space) - var(--control-box-radius-end))`},
				${x} ${y="calc(var(--tabs-top-space) * -1)"},
				${x=END_PC} ${y},
				${x} 100%
			);
			...
			prefs.tabsAtBottom
				? `:is(
					${prefs.tabsAtBottom == 1
						...
						: ``}
					:root[ai-window] #TabsToolbar:has(~ #notifications-toolbar notification-message),
					:root[aiwindow-immersive-view],
					#navigator-toolbox:has(~ #browser #ai-window-box:not([hidden]))
				)`
				: `:root[ai-window]`
		}
	...
	${prefs.tabsAtBottom ? `` : `:not(${hiddenMenubar}) +`} #TabsToolbar
		:is(
			${_}:not(${staticPreTabsPlaceHolder}) #tabs-placeholder-pre-tabs,
			#tabs-placeholder-post-tabs
		)
	...
	${_}:not([tabs-hide-placeholder], [pinned-tabs-wraps-placeholder])
		#tabbrowser-tabs:is([overflow], [positionpinnedtabs])
...
${!prefs.autoCollapse && prefs.maxTabRows > 1 ? /*css*/`
	:root {
	...
	${win7 || win8 ? `:root,` : ``}
	:root:not([ai-window]) #navigator-toolbox {
...
${_=`:root[multitabrows-applying-style] #navigator-toolbox`},
${_} > toolbar {
	transition: none !important;
}
...
if (splitViewProto) {
	...
	define(splitViewProto, {
		...
		multiselected: function() {
			let {selectedElements: n} = gBrowser;
...
let GET_DRAG_TARGET;
{
	...
	if (window.TabDragAndDrop) {
		...
		constructorString = constructorString.replace(
			/* startTabDrag, drag preview (fixed) */
			/*js*/ `let browser = isTab(tab) && tab.linkedBrowser;`,
			/*js*/ `let browser = isTabLike(tab) && (tab.tabs?.find(t => t.selected) ?? tab).linkedBrowser;`,
		).replace(
			/* getDropEffectForTabDrag (fixed) */
			/*js*/ `(isTab(sourceNode) || isTabGroupLabel(sourceNode))`,
			/*js*/ `(isTabLike(sourceNode) || isTabGroupLabel(sourceNode))`,
		).replace(
			/* _getDragTarget (fixed) */
			/*js*/ `isTab(target) || isTabGroupLabel(target)`,
			/*js*/ `isTabLike(target) || isTabGroupLabel(target)`,
		).replace(
			/* _getDropIndex, https://bugzil.la/2007720 (fixed) */
			/*js*/ `let item = this._getDragTarget(event);`,
			/*js*/ `let item = this._getDragTarget(event); item = item?.splitview ?? item;`,
		).replace(
			/* handle_drop, drop split view into collapsed group https://bugzil.la/2007737 */
			/isTabGroupLabel\(dropElement\)\s+&&\s+isTab\(draggedTab\)/,
			/*js*/ `isTabGroupLabel(dropElement) && isTabLike(draggedTab)`,
		).replace(
	...
	dragDropProto.multiselectStacking = getPref("browser.tabs.dragDrop.multiselectStacking", TAB_STACKING_SUPPORT);
	...
	dragDropProto.startTabDrag = function(e, tab, opt) {
		...
		let movingTabs = draggingTab && !opt?.fromTabList
			...
			: [tab];
		let stopAnimateTabMove = movingTabs.length > prefs.animateTabMoveMaxCount;
		let pauseAnimateTabMove = !stopAnimateTabMove && e.shiftKey && prefs.animateTabMoveShiftKeyToPause;
		...
		if (!draggingTab && !opt?.fromTabList)
	...
	dragDropProto._groupSelectedTabs = function(draggedTab, movingTabs) {
		...
		let tabIndex = selectedElements.indexOf(draggedTab);
		let shouldGroup = selectedElements.some(t => {
	...
	dragDropProto[`${HANDLE}_dragover`] = function(e) {
		...
		let draggingTabsOnly = isTab(draggedTab) && _dragData?.movingTabs.every(isTab); /* TODO pin/unpin splitview support? */
		...
		if (sameWindow && prefs.animateTabMoveShiftKeyToPause && !_dragData.stopAnimateTabMove)
			if (e.shiftKey) {
				if (!_dragData.pauseAnimateTabMove)
					_dragData.pauseAnimateTabMove = true;
				_dragData.fromTabList = true;
				...
				delete _dragData.pauseAnimateTabMove;
				_dragData.fromTabList = false;
			}
		...
		const allowToPin = draggingTabsOnly && !prefs.disableDragToPinOrUnpin;
	...
	dragDropProto._animateTabMove = async function(e) {
		...
		const pinDropInd = draggingTab && !prefs.hidePinnedDropIndicator && !movingTabs.some(t => !isTab(t))
		...
		if (lastX == null) {
			...
			assign(_dragData, {
				...
				moveOverThreshold,
				groupThreshold,
				createGroupDelay: dragToGroupTabs && getPref("browser.tabs.dragDrop.createGroup.delayMS"),
			});
		...
		let shiftMovingNodes = () => {
			...
			cursorRow = Math.min(
				Math.max(
					Math.floor(
						(draggedRect.y + draggedRect.height / 2 + tranY + scrollPos + scrollChange - boxRect.y)
						/ tabHeight
					),
					firstRect.row,
				),
	...
	dragDropProto._postDraggingCleanup = function() {
		...
		if (!tabContainer.isFinishingTabMove && !tabContainer.isMovingTab) {
			/*
			since the stacking of group is triggered by dragstart, the drop may
			start before the first dragover, need to clean up the stacking in this case
			*/
			let stackedGroup = gBrowser.tabGroups?.find(g => g.stacked);
			if (stackedGroup) {
				stackedGroup.stacked = false;
				tabContainer._unlockTabSizing({instant: true, unlockSlot: false});
			}
		}
...
/** tabContainer **/
{
	...
	tabContainer._positionPinnedTabs = function(numPinned, recursion) {
		if (!prefs.positionPinnedTabs && !this.positioningPinnedTabs)
			return;
		...
		if (floatPinnedTabs) {
			...
			if (!isPositioned) {
				let lastTabRect = getRect(lastNode);
	...
	*/
	tabContainer._hideFakeScrollbar = function() {
		...
		let slotLocked = getStyle(arrowScrollbox, "--slot-height");
		if (overflowing)
			this.lockSlotSize();
		style(fakeScrollbar, {"--slot-height": ""});
		this.removeAttribute("tabmousedown");
		if (overflowing && !slotLocked)
			this.unlockSlotSize();
	};
	...
	tabContainer.updateLayout = function() {
		...
		if (
			...
			let winMaxWidth = Math.max(screen.width + 8, winWidth + maxItemWidth);
			...
			let css = [/*css*/`
				${media(0, base + firstNodeMinWidth + (adjacentNewTab ? newTabButtonWidth : 0) - EPSILON)} {
					...
					#tabbrowser-tabs${condition} #tabbrowser-arrowscrollbox
						:is(
							.tabbrowser-tab,
							.tab-group-label-container,
							.tab-group-overflow-count-container,
	...
	tabContainer.addEventListener("TabGroupUpdate", function(e) {
		this.forcedOverflow = 0;
		this._unlockTabSizing({instant: true, ignorePanels: true});
	...
	tabContainer.unlockSlotSize = function(instant) {
...
/** gBrowser **/
{
	...
	gBrowser.addTab = function(uri, o) {
		...
		let action = () => {
			...
			//because of the fix of https://bugzil.la/1997096, multiple nodes may become visible after creating a tab
		...
		try {
			animateLayout(
				uri?.match(/^about:firefoxview#?/i)
					// let the animation run in subprocess to
					// wait for the firefox view tab to be hidden
					? async () => action()
					: action,
		...
		if (animatingLayout || !this.visibleTabs.includes(tab) || gReduceMotion)
			...
			animateLayout(() => {
				...
				if (!tab.closing || !tab.isConnected) {
					tabContainer.updateLayout();
					if (animatingLayout && closingLast) {
						let newTab = gBrowser.visibleTabs[0];
						if (newTab) {
							animatingLayout.rects.set(newTab, animatingLayout.rects.get(tab));
							animatingLayout.nodes.push(newTab);
						}
					}
					return;
				}
...
let tabsResizeObserver = new ResizeObserver(() => {
	...
	if (animatingLayout || tabContainer._unlockingTabs)
		return;
...
if (prefs.smartWindowButtonOnNavBar && $("#ai-window-toggle"))
	if (AIWindow._windowStates.has(window))
		updateAIWindowSwitcherPosition();
	else
		new MutationObserver((mutations, observer) => {
			if (mutations.some(m => [...m.addedNodes].some(n => n.id == "ai-window-toggle"))) {
				observer.disconnect();
				updateAIWindowSwitcherPosition();
			}
		}).observe(tabsBar, {childList: true});
...
function updateAIWindowSwitcherPosition() {
	let btn = $("#ai-window-toggle");
	if (btn) {
		$(`
			#${prefs.smartWindowButtonOnNavBar ? "nav-bar" : "TabsToolbar"}
				.titlebar-buttonbox-container
		`).before(btn);
		updateNavBarOverflow();
	}
...
function updateThemeStatus() {
	...
	micaEnabled = getPref("widget.windows.mica", false);
...
async function animateLayout(
	...
	return new Promise(async rs => {
		...
		if (deltaNodes[0]) {
			if (
				deltaNodes.find(isTabGroupOverflowContainer)
					?.closest("tab-group").getAttribute("toggling")
				!= "collapse"
			)
				tabContainer._unlockTabSizing({instant: true, unlockSlot: false});
